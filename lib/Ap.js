...
console.log('TODO {parPtr matAB a b} {locPtr matAB a b} (matAB parlocMem ...) etc');
/*
...
const Ap = {
	...
	todos:[
		...
		'let rvalue = forceInt ? this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt) : this.childs[1].floatCode(tablev+1).trim(); //TODO merge floatCode and toTinyGlslCodeRecurse and the forceInt param.',
	...
	*/
	...
	//a writable and readable Float32Array that has a list of parlocMem locparMem and parMem apes.
	...
	Mem: function(apeType, childs){
		...
		if(this.memSize === undefined){
			throw 'Some size is not an integer cuz total size is undefined';
		}
	...
	Call: function(ape, par, loc, big, beforeCode, code, numGpuThreads, floatsPerGpuThread){
		this.ape = ape;
		this.par = par;
		this.loc = loc;
		this.big = big;
...
Ap.Ape.prototype.secondPassOfUpdateApeSize = function(){
	if(this.containsParLocOrBig){
		this.childs.forEach(child=>child.secondPassOfUpdateApeSize());
		this.updateApeSize();
	}
	return this;
};
...
Ap.Call.prototype.copy = function(){
	throw 'TODO copy the Ap.Mem objects too return new Ap.Call(this.ape, mem: Ap.copyFloatsOrNull(par), ... loc, mem: big, beforeCode, code, numGpuThreads, floatsPerGpuThread){';
};
...
Ap.Call.prototype.eval = function(){
	...
	return TinyGlsl.simple(this.beforeCode, this.code, this.par.floats, this.big.floats, h, w, this.floatsPerGpuThread);
};
...
Ap.Mem.prototype.putFloats = function(apeOrName, floats){
	if(floats.length != this.size(apeOrName)){
		throw 'floats.length='+floats.length+' but this.size(apeOrName)='+this.size(apeOrName);
	}
...
Ap.Mem.prototype.testGet = function(arrayName, index, expectVal){
	let observedVal = this.get(arrayName, index);
	if(isNaN(observedVal)!=isNaN(expectVal) || (observedVal===undefined)!=(expectVal===undefined) || Math.abs(observedVal-expectVal)>Ap.floatEpsilonForTestGet){
		throw 'Mem.testGet FAIL memType='+this.apeType+' arrayName='+arrayName+' index='+index+' expectVal='+expectVal+' observedVal='+observedVal+' floatEpsilonForTestGet='+Ap.floatEpsilonForTestGet;
	}
	return this;
};
...
Ap.Ape.prototype.transformNames = function(renamer, allowNullName){
	...
	return new Ap.Ape(name, this.apeType, this.childs.map(c=>c.transformNames(renamer)), this.data);
};
...
Ap.lazyEval = function(apeCode){
	...
	return Ap.parse(apeCode).toApe().toMem();
};
/*
...
const Todo = function(str){
	throw new Error(str || 'TODO');
};

const Err = function(str){
	throw new Error(str || '(Err with no message)');
};
...
Ap.Ape.prototype.updateApeSize = function(){
	...
	switch(this.apeType){
		case 'floatVal':{
		...
		}break;case 'par':{ //apCall.par shared array. //{par} contains all {parMem} and {parlocMem} and {locparMem}.
			this.apeSize = Ap.singleThreadedSharedTempVars.parSize; //this.computeSizeOfSharedParArray();
			...
			for(let c of this.childs){
				let cSiz = c.size();
				if(cSiz === 0){
					throw 'Child size is 0 in a mem. this='+this+' child='+c;
				}
				this.apeSize *= c.size();
		...
		*/
			...
			if(this.childs.length != 1){
				throw 'apeType=int must have 1 child (what its int value is set to) but has this many: '+this.childs.length;
			}
			this.apeSize = this.childs[0].size();
		}break;case 'size':{
			if(this.childs.length != 1){
				throw 'apeType=size must have 1 child (what it gets the ape.size() of) but has this many: '+this.childs.length;
			}
			this.apeSize = this.childs[0].size();
		}break;case 'imod':{
			if(this.childs.length != 2){
				throw 'apeType=imod must have 2 childs but has this many: '+this.childs.length;
			}
		...
		}break;case 'i==':case 'f*':case 'f+=':case 'f+':case 'f-':case 'f/':case 'f**':case 'fsigmoid':case 'f=':case 'ptr':case 'parPtr':case 'locPtr':case 'bigPtr':
				case 'fhypot':case 'fless':case 'fmore':case '?:':case 'fxtanh':case 'fmod':case 'freturn':case 'fsin':{
			...
			this.apeSize = 1; //+1 for this calculation after childs return
			for(let child of this.childs){
			...
			if(iter.size() != listSize){
				throw 'listLoop apeType must have iter.size() == listSize. iter.size()='+iter.size()+' listSize='+listSize;
			}
	...
	if(typeof(this.apeSize) != 'number' || isNaN(this.apeSize)){
		throw new Error('apeSize must be a number but is typeof='+typeof(this.apeSize)+': '+this.apeSize+' ape='+this);
	}
...
Ap.Ape.prototype.sharedArrayName = function(){
	...
	if(array.apeType=='parMem' || array.apeType=='parlocMem' || array.apeType=='par'){
		...
		throw 'TODO remove localparMem and parrMem';
...
Ap.Ape.prototype.outermostIsDeclaringArrayMem = function(){
...
Ap.Ape.prototype.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc = function(tablev, opString, isInt){
	let code = '';
	...
	for(let i=0; i<this.childs.length; i++){
		...
		if(isInt){
			if(multiLine) code += Ap.nline(tablev+1)+ch.toTinyGlslCodeRecurse(tablev+1,isInt);
			else code += ch.toTinyGlslCodeRecurse(0,isInt).trim();
		}else{
			if(multiLine) code += Ap.nline(tablev+1)+ch.floatCode(tablev+1); //floatCode puts float( ... ) around it if its int.
			else code += (i>0 ? Ap.nline(tablev+1) : '')+ch.floatCode(0).trim();
		}
	}
...
//isInt defaults to false if dont give that param.
Ap.Ape.prototype.toTinyGlslCodeRecurse_tanhPowSinExpEtc = function(tablev, glslFuncName, isInt){
	//return glslFuncName+'('+this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev, ',')+')';
	return glslFuncName+Ap.wrapInParensIfNotAlready(this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev, ',', isInt));
};
...
Ap.Ape.prototype.toMem = function(optional_isSecondPass){ //FIXME rename Ape.toMem to toCall since it returns an Ap.Call not an Ap.Mem (Call contains 3 Mems).
	...
	if(bigOnlyMems && bigOnlyMems.length){
		throw 'TODO bigMem. As of 2023-11-3 tinyGlsl has a bigMem/big param which will be a Float32Array but its ignored.';
	}

	//FIXME remove parrMem and localparMem. The new mem types are: parMem locMem locparMem parlocMem float bigMem.
	let parrMems = this.searchApes(ape=>(ape.apeType=='parrMem')); //TODO Removing these
	let localparMems = this.searchApes(ape=>(ape.apeType=='localparMem')); //TODO Removing these
	if(parrMems.length){
		throw 'Use locMem or locparMem or parlocMem, instead of parrMem.';
	}
	if(localparMems.length){
		throw 'Use locMem or locparMem or parlocMem, instead of localparMem.';
	}
	...
	for(let overlappingParAndLocMem of overlappingParAndLocMems){
		...
		code += Ap.nline(tablev)+'const int '+overlappingParAndLocMem.OffsetName()+' = '+offset+'; //array offset in parMem and locMem ('+apeType+')';
		code += Ap.nline(tablev)+'const int '+overlappingParAndLocMem.SizeName()+' = '+size+'; //array size in parMem and locMem ('+apeType+')';
		offset += size;
	...
	if(offset != overlappingParAndLocMems_sizeInFloats){
		throw 'offset='+offset+' but overlappingParAndLocMems_sizeInFloats='+overlappingParAndLocMems_sizeInFloats;
	}
	...
	for(let parOnlyMem of parOnlyMems){
		...
		code += Ap.nline(tablev)+'const int '+parOnlyMem.OffsetName()+' = '+offset+'; //array offset in parMem  (after overlapping par and loc mems)';
		code += Ap.nline(tablev)+'const int '+parOnlyMem.SizeName()+' = '+size+'; //array size in parMem  (after overlapping par and loc mems)';
		offset += size;
		if(offset > 3000){ //FIXME get this var from somewhere, maybe in TinyGlsl. max something (var name). As of 2023-10-14 tinyGlsl doesnt have a localparMem or bigMem, just parrMem.
			throw 'parOnlyMemOffset='+offset+' so far, which is too big. the last bigMemApe is '+bigMemApe;
		}
	}
	if(offset != overlappingParAndLocMems_sizeInFloats+parOnlyMems_sizeInFloats){
		throw 'offset('+offset+') != overlappingParAndLocMems_sizeInFloats('+overlappingParAndLocMems_sizeInFloats+')+parOnlyMems_sizeInFloats('+parOnlyMems_sizeInFloats+')';
	}
	...
	for(let locOnlyMem of locOnlyMems){
		...
		code += Ap.nline(tablev)+'const int '+locOnlyMem.OffsetName()+' = '+offset+'; //array offset in locMem (after overlapping par and loc mems)';
		code += Ap.nline(tablev)+'const int '+locOnlyMem.SizeName()+' = '+size+'; //array size in locMem  (after overlapping par and loc mems)';
		offset += size;
		if(offset > 3000){ //FIXME get this var from somewhere, maybe in TinyGlsl. max something (var name). As of 2023-10-14 tinyGlsl doesnt have a localparMem or bigMem, just parrMem.
			throw 'parOnlyMemOffset='+offset+' so far, which is too big. the last bigMemApe is '+bigMemApe;
		}
	...
	if(offset != overlappingParAndLocMems_sizeInFloats+locOnlyMems_sizeInFloats){
		throw 'offset != overlappingParAndLocMems_sizeInFloats+locOnlyMems_sizeInFloats';
	}
	...
	for(let bigMemApe of bigOnlyMems){
		...
		code += Ap.nline(tablev)+'const int '+bigMemApe.OffsetName()+' = '+offset+'; //array offset in bigMem';
		code += Ap.nline(tablev)+'const int '+bigMemApe.SizeName()+' = '+size+'; //array size in bigMem';
		offset += size;
	}
	if(offset != bigOnlyMems_sizeInFloats){
		throw 'offset != bigOnlyMems_sizeInFloats';
	}
	...
	for(let intMemApe of intOnlyMems){
		if(intMemApe.childs.length != 1){
			throw 'Theres no child of the intMemApe. Thats supposed to be the code that sets its value in the same line of code that creates the int var in glsl.';
		}
		...
		code += Ap.nline(tablev)+'int '+intMemApe.LoopName()+' = '+intMemApe.childs[0].toTinyGlslCodeRecurse(tablev).trim()+';';
		//TODO? code += Ap.nline(tablev)+'int '+intMemApe.SizeName()+' = '+intMemApe.childs[0].toTinyGlslCodeRecurse(tablev).trim()+';';
	}
	...
	code += Ap.nline(tablev)+'const int par_o = 0; //view of whole {par} shared array';
	code += Ap.nline(tablev)+'const int loc_o = 0; //view of whole {loc} shared array';
	code += Ap.nline(tablev)+'const int big_o = 0; //view of whole {big} shared array';

	code += Ap.nline(tablev)+'const int par_s = '+par_sizeInFloats+'; //size of {par} shared array';
	code += Ap.nline(tablev)+'const int loc_s = '+loc_sizeInFloats+'; //size of {loc} shared array';
	code += Ap.nline(tablev)+'const int big_s = '+big_sizeInFloats+'; //size of {big} shared array';
	...
	let loc = new Ap.Mem('loc', locMems); //loc only exists in GPU, not in input or output.
	let big = new Ap.Mem('big', bigMems);
	/*return {
		type: 'ape_memsMap',
		tinyglslBeforeCode: beforeCode,
		tinyglslCode: code,
	...
	*/
	...
	if(!optional_isSecondPass && this.containsParLocOrBig){
		...
		return thisApe.toMem(true); //recurse: optional_isSecondPass is true in that call so it wont recurse again here.
...
//If its already a float, leaves it as it is.
Ap.Ape.prototype.floatCode = function(tablev){
	let r = this.toTinyGlslCodeRecurse(tablev);
	if(this.evalsToInt()){
		let trimmed = r.trim();
...
Ap.Ape.prototype.evalsToInt = function(){
	...
	return this.apeType=='int' || this.apeType=='id' || this.apeType=='idy' || this.apeType=='idx' || this.apeType=='i%s' ||
		this.apeType=='imod' || this.apeType=='i/' || this.apeType=='i+' || this.apeType=='i-' || this.apeType=='copy'; //dont include 'i==' cuz that evals to true/false.
};
...
*/
Ap.Ape.prototype.toTinyGlslCodeRecurse = function(tablev, forceInt){
	let code = '';
	if(this.apeType == '*'){
		...
		code += this.childs[lastIndex].toTinyGlslCodeRecurse(tablev+lastIndex,forceInt);
		...
		code += this.childs[2].toTinyGlslCodeRecurse(tablev+1,forceInt);
		...
		let beforeEach = this.childs[1];
		let afterEach = this.childs[2];
		let beforeEachStr = Ap.nline(tablev)+beforeEach.toTinyGlslCodeRecurse(tablev,forceInt);
		let afterEachStr = Ap.nline(tablev)+afterEach.toTinyGlslCodeRecurse(tablev,forceInt);
		...
		let listIndex = 0;
		for(let i=3; i<this.childs.length; i++){
			code += Ap.nline(tablev)+(i==3?'int ':'')+iter.LoopName()+' = '+(listIndex++)+'; //listLoop iter '+iter;
			code += beforeEachStr;
			code += Ap.nline(tablev)+this.childs[i].toTinyGlslCodeRecurse(tablev,forceInt).trim();
			code += afterEachStr;
		}
	}else if(this.outermostIsDeclaringArrayMem()){
		code +=  Ap.nline(tablev)+'//outermostIsDeclaringMem apeType='+this.apeType+' name='+this.Name();
	}else if(this.apeType == '+'){
		for(let child of this.childs){
			if(child.outermostIsDeclaringMem()){ //FIXME this shouldnt be specific to "else if(this.apeType == '+')" and childs. Move it parallel to that.
				code +=  Ap.nline(tablev)+'//in {+ ...}, outermostIsDeclaringMem '+child.Name();
			}else{
				code += child.toTinyGlslCodeRecurse(tablev,forceInt);
			}
		}
	}else if(this.apeType == 'f='){
		...
		let rvalue = forceInt ? this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt) : this.childs[1].floatCode(tablev+1).trim(); //TODO merge floatCode and toTinyGlslCodeRecurse and the forceInt param.
		code += Ap.nline(tablev)+this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt)+' = '+rvalue+';'; //FIXME one line or multiline depending on length
	}else if(this.apeType == 'f+='){
		code += Ap.nline(tablev)+this.childs[0].ptrCode()+' += '+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+';'; //FIXME make a func that does it in 1 line
	}else if(this.apeType == 'f*='){
		//FIXME if more params, * them all like in f*
		code += Ap.nline(tablev)+this.childs[0].ptrCode()+' *= '+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+';'; //FIXME make a func that does it in 1 line
	}else if(this.apeType == 'f/='){
		code += Ap.nline(tablev)+this.childs[0].ptrCode()+' /= '+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+';';
	}else if(this.apeType == 'f+'){
		...
		*/
		...
		code += '(1./(1.+exp(-'+this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+')))';
		...
		code += '('+this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+'<'+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+')';
		...
		code += '('+this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+' ? '+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+' : '+this.childs[2].toTinyGlslCodeRecurse(tablev+1).trim()+')';
		...
		*/
		code += '(((temp1 = '+this.childs[0].toTinyGlslCodeRecurse(tablev+1,forceInt).trim()+')<1.) ? (tanh(temp1+1.)-1.) : ((temp1>1.) ? (tanh(temp1-1.)+1.) : temp1))';
	...
	//	code += '//(((temp1 = '+this.childs[0].toTinyGlslCodeRecurse(tablev+1).trim();+')<1.) ? (tanh(temp1+1.)-1.) : ((temp1>1.) ? (tanh(temp1-1.)+1.) : temp1))';
		...
		//TODO bring this back after i put ape.floatCode(tablev) in a more general place that affects all the code at once?
		/*
		You
		is it mod or fmod
		ChatGPT
		...
		*/
		code += this.toTinyGlslCodeRecurse_tanhPowSinExpEtc(tablev, 'mod'); //FIXME check for divide by 0? or just let it become NaN or plus/minus infinity?
		...
		code += Ap.nline(tablev)+this.childs[0].toTinyGlslCodeRecurse(tablev,forceInt)+'/'+this.childs[1].toTinyGlslCodeRecurse(tablev+1,forceInt).trim();
		...
		code += TinyGlsl.jsNumToGlslFloat(this.data.floatVal);
		...
		if(this.childs.length == 1){
			//code += Ap.nline(tablev)+'fragColor = vec4('+this.childs[0].toTinyGlslCodeRecurse(0).trim()+',0.,0.,0.); //return 1 float later';
			code += Ap.nline(tablev)+'ret = '+this.childs[0].toTinyGlslCodeRecurse(0,forceInt).trim()+';';
		}else if(this.childs.length == 4){
			//code += Ap.nline(tablev)+'fragColor = vec4('+this.childs[0].toTinyGlslCodeRecurse(0).trim()+','+this.childs[1].toTinyGlslCodeRecurse(0).trim()+','+this.childs[2].toTinyGlslCodeRecurse(0).trim()+','+this.childs[3].toTinyGlslCodeRecurse(0).trim()+'); //return 4 floats later';
			code += Ap.nline(tablev)+'ret = '+this.childs[0].toTinyGlslCodeRecurse(0,forceInt).trim()+';';
			code += Ap.nline(tablev)+'retb = '+this.childs[1].toTinyGlslCodeRecurse(0,forceInt).trim()+';';
			code += Ap.nline(tablev)+'retc = '+this.childs[2].toTinyGlslCodeRecurse(0,forceInt).trim()+';';
			code += Ap.nline(tablev)+'retd = '+this.childs[3].toTinyGlslCodeRecurse(0,forceInt).trim()+';';
		...
		//OLD: its mod, not %: code += this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev,'%').trim();
		//throw 'FIXME this is for floats. do ints.';

		//Its int%int or mod(float,float). Since this (imod) is ints, use %.
		code += this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev,'%',true).trim(); //true isInt
		//code += this.toTinyGlslCodeRecurse_tanhPowSinExpEtc(tablev,'mod',true).trim(); //true isInt
		...
		code += this.childs[this.childs.length-1].toTinyGlslCodeRecurse(tablev,forceInt); //last child
	}else if(this.apeType=='ptr' || this.apeType=='parPtr' || this.apeType=='locPtr' || this.apeType=='bigPtr'){
		if(this.childs.length < 2){
			throw 'ptr. Must be at least 2 childs (arrayName and index, or can have multiple indexs or other ptr arithmetic) is '+this.childs.length;
		}
	...
	}else if(this.apeType == 'x'){ //FIXME remove x and y and z cuz they are dagball specific
...
Ap.ave = function(floats){
	if(!floats || !floats.length) throw 'No floats to average';
	let sum = 0;
...
Ap.stdDev = function(floats, optionalAve){
	if(!floats || !floats.length) throw 'No floats to stdDev';
...
Ap.doComplexApeTests = function(){
	...
	let apeForTestingSharedParArraySize = Ap.lazyEval('{doLast (arrayX parMem {3}) (arrayY parMem {4}) {* (ten 10) {par}}}').ape;
	...
	let matmulCall = Ap.lazyEval(matmulCode);
	...
	*/
	...
	Ap.lazyEval('{+ <(b float) 1> {* {17} {+ <(next float) {f+ (a float) b}> <a b> <b next>}} {freturn b}}').evalTest().testGet(0,2584).log('Test pass: fibonacci');
	...
	*/
	...
	Ap.lazyEval('{freturn 2 20 200 2000}').setThreadsFloats(1,4).evalTest().testGet(0,2).testGet(1,20).testGet(2,200).testGet(3,2000).
	...
	*/
	...
	//Ap.lazyEval('{freturn 2 20 200 2000}').setThreadsFloats(1,4).evalTest().
