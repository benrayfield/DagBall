...
const Ap = {
	...
	*/
	...
	Call: function(ape, par, loc, big, vel, beforeCode, code, numGpuThreads, floatsPerGpuThread){
		this.ape = ape;
...
Ap.Call.prototype.eval = function(){
	if(this.ape.containsGpucpu){ //compile to javascript and run in CPU. This is for vel dvel and dpos differentialEquation adjustments for gameplay.
		...
		*/
		...
		let jsFunc = TinyGlsl.cachedJsEval(jsCode); //FIXME might need to wrap this in (params)=>{...} etc.
		...
		jsFunc(newPar, newLoc, newBig, newVel, newDvel, newDpos, newDfriction);
...
Ap.Ape.prototype.updateApeSize = function(){
	...
	switch(this.apeType){
		...
		*/
			...
			case 'f**':case 'fsigmoid':case 'f=':case 'ptr':case 'parPtr':case 'locPtr':case 'bigPtr':case'dposPtr':case 'velPtr':case 'dvelPtr':case 'dfrictionPtr':
				case 'fhypot':case 'fless':case 'fmore':case 'fmax':case 'fmin': case 'fatan': case 'gpucpu': case 'velPtr': case 'dvelPtr': case 'dposPtr':
				case '?:':case 'ftanh':case 'fxtanh':case 'fmod':case 'freturn':case 'flog':case 'fsin':case 'fcos':case 'fabs':case 'fexp':case '?':case 'funk':case '@':case 'funcReturn':{
			//renamed 'call' to '@'
...
Ap.Ape.prototype.toMem = function(optional_isSecondPass){ //FIXME rename Ape.toMem to toCall since it returns an Ap.Call not an Ap.Mem (Call contains 3 Mems).
	...
	if(loc_sizeInFloats){
		...
		if(Ap.fillLocWithAll0sRightAfterCreate){
			code += Ap.nline(tablev)+'for(int i=0; i<'+loc_sizeInFloats+'; i++) loc[i] = 0.; //Ap.fillLocWithAll0sRightAfterCreate';
		}
	}
	...
	for(let overlappingParAndLocMem of overlappingParAndLocMems){
		...
		beforeCode += Ap.nline(tablev)+'const int '+overlappingParAndLocMem.OffsetName()+' = '+offset+'; //array offset in parMem and locMem ('+apeType+')';
		beforeCode += Ap.nline(tablev)+'const int '+overlappingParAndLocMem.SizeName()+' = '+size+'; //array size in parMem and locMem ('+apeType+')';
		offset += size;
	...
	for(let parOnlyMem of parOnlyMems){
		...
		beforeCode += Ap.nline(tablev)+'const int '+parOnlyMem.OffsetName()+' = '+offset+'; //array offset in parMem  (after overlapping par and loc mems)';
		beforeCode += Ap.nline(tablev)+'const int '+parOnlyMem.SizeName()+' = '+size+'; //array size in parMem  (after overlapping par and loc mems)';
		offset += size;
		if(offset > 3000){ //FIXME get this var from somewhere, maybe in TinyGlsl. max something (var name). As of 2023-10-14 tinyGlsl doesnt have a localparMem or bigMem, just parrMem.
	...
	for(let locOnlyMem of locOnlyMems){
		...
		beforeCode += Ap.nline(tablev)+'const int '+locOnlyMem.OffsetName()+' = '+offset+'; //array offset in locMem (after overlapping par and loc mems)';
		beforeCode += Ap.nline(tablev)+'const int '+locOnlyMem.SizeName()+' = '+size+'; //array size in locMem  (after overlapping par and loc mems)';
		offset += size;
		if(offset > 3000){ //FIXME get this var from somewhere, maybe in TinyGlsl. max something (var name). As of 2023-10-14 tinyGlsl doesnt have a localparMem or bigMem, just parrMem.
			throw 'parOnlyMemOffset='+offset+' so far, which is too big. the last bigMemApe is '+bigMemApe;
		}
	...
	for(let bigMemApe of bigOnlyMems){
		...
		beforeCode += Ap.nline(tablev)+'const int '+bigMemApe.OffsetName()+' = '+offset+'; //array offset in bigMem';
		beforeCode += Ap.nline(tablev)+'const int '+bigMemApe.SizeName()+' = '+size+'; //array size in bigMem';
		offset += size;
	}
	...
	for(let floatMemApe of floatOnlyMems){
		...
		beforeCode += Ap.nline(tablev)+'float '+floatMemApe.FloatName()+' = 0.;';
	...
	beforeCode += Ap.nline(tablev)+'const int par_o = 0; //view of whole {par} shared array';
	beforeCode += Ap.nline(tablev)+'const int loc_o = 0; //view of whole {loc} shared array';
	beforeCode += Ap.nline(tablev)+'const int big_o = 0; //view of whole {big} shared array';
	if(includeDiffeqMems){
		beforeCode += Ap.nline(tablev)+'const int vel_o = 0; //readonly. view of whole {vel} shared array. normally only used in CPU.';
		beforeCode += Ap.nline(tablev)+'const int dvel_o = 0; //readwrite. view of whole {dvel} shared array. normally only used in CPU.';
		beforeCode += Ap.nline(tablev)+'const int dpos_o = 0; //readwrite. view of whole {dpos} shared array. normally only used in CPU.';
	}

	beforeCode += Ap.nline(tablev)+'const int par_s = '+par_sizeInFloats+'; //size of {par} shared array';
	beforeCode += Ap.nline(tablev)+'const int loc_s = '+loc_sizeInFloats+'; //size of {loc} shared array';
	beforeCode += Ap.nline(tablev)+'const int big_s = '+big_sizeInFloats+'; //size of {big} shared array';
	if(includeDiffeqMems){
		...
		beforeCode += Ap.nline(tablev)+'const int vel_s = '+overlappingParAndLocMems_sizeInFloats+'; //readonly. size of {vel} shared array. normally only used in CPU.';
		beforeCode += Ap.nline(tablev)+'const int dvel_s = '+overlappingParAndLocMems_sizeInFloats+'; //readwrite. size of {dvel} shared array. normally only used in CPU.';
		beforeCode += Ap.nline(tablev)+'const int dpos_s = '+overlappingParAndLocMems_sizeInFloats+'; //readwrite. size of {dpos} shared array. normally only used in CPU.';
		...
		if(Ap.fillLocWithAll0sRightAfterCreate){
			code += Ap.nline(tablev)+'for(int i=0; i<'+overlappingParAndLocMems_sizeInFloats+'; i++){ //Ap.fillLocWithAll0sRightAfterCreate';
	...
	if(par_sizeInFloats != overlappingParAndLocMems_sizeInFloats){
		console.log('WARNING: TODO this has not been tested much and will likely break. see the gradient code in codeMaker that loops over par and loc.');
	}
	...
	*/
	...
	let thisApe = this;
	let makeCall = function(){
		return new Ap.Call(thisApe, par, loc, big, vel, beforeCode, code, numGpuThreads, floatsPerGpuThread);
	};
...
Ap.Call.prototype.toCode = function(lang){
	...
	//try{
		...
		if(lang=='js'){
			...
			let code = prefix+this.beforeCode+mid+this.code+suffix;
			//beforeCode += Ap.nline(tablev)+'float loc['+loc_sizeInFloats+']; //contains all locMem, locparMem, parlocMem arrays. ptrLoc or ptr can point into here.';
...
Ap.Ape.prototype.toTinyGlslCodeRecurse = function(lang, tablev, forceInt){
	...
	//}else if(this.apeType == '+'){ //V2 syntax of + is (do ...), a list of statements to do in order. similar to * in V1 is (oo counterA counterB counterC... loopBody) in V2.
	}else if(this.apeType == '+' || this.apeType == 'gpucpu'){
		...
		*/
		...
		code += Ap.nline(tablev)+'//Not defining '+this.apeType+' here cuz it goes before main(): '+this.Name()+'('+listOfParamNames.join(',')+')';
...
Ap.doComplexApeTests = function(){
	...
	console.log('Ap.Ape NEW/NEXT LANGUAGE tests pass');
