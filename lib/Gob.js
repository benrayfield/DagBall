...
Requires Dagverse.js/DV be loaded first.
...
...
[[In theory, video game rules (physics+gameplay) can sync at lower lag and scale up in peer to peer by only using rules
that have same effect (except roundoff) regardless of order rules are applied, including loops in loops,
aka order invariant. Astronomical number of automorphisms
...
TODO In Gob.js any gob (game object) can be a game rule that acts on pairs of gobs near it. You could paint
newtonian gravity onto a huge sphere in a massively multiplayer 3d game world,
and paint mandelbrot fractal (gobs stay inside or outside the mandelbrot shaped area), etc
]]
*/
...
const GB = (()=>{
	...
	let Gob = GB.Gob = function(sim, optionalUp, optionalL, optionalR, optionalP, optionalV){
		...
		//this.ch = []; //TODO move ch into this.w.rulCh?
		//2024-3-4 TODO moving this.ch to this.w.rulCh, since only rules should have it.

		//for Octree. Cache of this.m.x.p if this.m.x exists, else 0. same for x y z r.
		...
		//TODO update this in gob.doPhysics(dt) (UPDATE: no, do it in gob.updateOctree() to be called after all relevant doPhysics calls),
		//remove from this.sim.oct, change ox oy oz or, then add back.
		...
		this.or = 0;
		this.sim = sim;
		//parent pointer, if this is a field of another gob, else null.
		//Also, maybe namespace/sim should be the parent of 'leaf gobs' that go in its namespace.m?
		this.up = optionalUp || null;
		this.l = optionalL || null; //left/func child, like in wikibinator despite gobs are mutable sparse dimensional vectors
		this.r = optionalR || null; //right/param child, like in wikibinator despite gobs are mutable sparse dimensional vectors
		//curries left before eval, in this.l/func and this.r/param. See this.Q(...gobParams) for currying.
		//If you give it less than this.cur params youll get a tree of function/gob calls whose .l and .r are such curries like in wikibinator.
		this.cur = 0;
		//sim.gobs.add(this);
		sim.add(this);
	};
	...
	Gob.prototype.rad = function(){
		let field = this.m.rad;
		return field ? field.p : 0;
	};

	//Similar to this.L() and this.R(), but defaults to this gob if there is no this.up (FIXME?).
	Gob.prototype.UP = function(){
		return this.up || this;
	};

	//In wikibinator, forall x, (L x (R x)) equals x. If this.l is null this therefore returns
	//a gob that is an identityFunc, so when called on this.R() returns this.
	Gob.prototype.L = function(){
		return this.l || this.sim.identityFunc; //FIXME create sim.identityFunc in sim constructor as some gob whose rule is that.
	};

	//In wikibinator, forall x, (L x (R x)) equals x. If this.r is null this therefore returns
	//this gob so when this.L() aka identityFunc is called on this.R() it returns this.
	Gob.prototype.R = function(){
		return this.r || this;
	};

	/*
	//call this gob on gobParam as a combinator, which may modify this andOr gobParam, and returns a number (lowercase q, capital Q returns a gob).
	Gob.prototype.q = function(gobParam){
		throw new Error('TODO');
	};

	//call this gob on gobParam as a combinator, which may modify this andOr gobParam, and returns a gob (capital Q, lowercase q returns a number).
	//If theres more params to wait on, just returns a gob whose l is this and r is gobParam, that you can further Q call to give more params.
	Gob.prototype.Q = function(gobParam){
		throw new Error('TODO');
	};*/
	//TODO by vararg gobParams im going for efficiency of not having to curry gobParams[0] then call that on gobParams[1] and so on.
	//Use infl (influence) of 0 for this to not modify dp dv kv ap av zp zv grad etc,
	//but may still add gobs to sim if what it needs to return (by its combinator logic) is not yet in the sim.
	//FIXME maybe should generalize infl to something like StackStuff object in wikibinator but mutable, more like the vars that count compute and memory etc in wikib?
	Gob.prototype.Q = function(infl, ...gobParams){
		throw new Error('TODO');
	};
	...
	//TODO make this a js field in Sim?
	...
	GB.maxGobsPerOctreeNodeIf = 30;
	
	//namespace is a small string that starts with '$' such as '$testNs567'. Each sim is a high dimensional vector with various other objects
	...
	Sim.prototype.add = function(gob){
		this.oct.add(gob);
	};
...
`gob=>{ //(thebNearxyzrEachc s$3456 3.1 //the B each C, where xyz distance + the 2 radiuses <= 3.1
	...
	*/
	...
	Gob.prototype.M = function(key,optionalDefaultP,optionalDefaultV){
		return this.m[key] || (this.m[key] = this.sim.newGob(this,optionalDefaultP,optionalDefaultV));
	};
	...
	Gob.prototype.toMap = function(skipNorm){
		...
		if(!big){
			//this is just a position and velocity, and everything else
			//is the defaults or derived (from current state or recent out of sync state, FIXME?).
			//TODO check for this case and dont even create the {... if !isBig, making less heap mem but more compute cycles, which is good for cache}.
	...
	GB.Sim.prototype.mapToGob = function(map){
		let gob = this.newGob(null); //FIXME should the up/parentPtr be null here?
		if(map.length){
			gob.p = map[0];
			gob.v = map[1];
		}else{
	...
	//updates this.p and this.v based on this.dp (for chuasCircuit etc) dv (accel) kv (velocityDecayRate) ap zp av zv gr (gradient) and lock.
	...
	Gob.prototype.doPhysics = function(dt){
		if(!this.lock){
			let nextP = this.p+dt*this.dp+dt*this.v;
			let nextV = this.v*Math.exp(-dt*this.kv)+dt*this.dv-dt*this.gr; //TODO rename gr to grad? in case g and r somehow combine to make it confusing?
			if(this.ap <= this.zp){
	...
	Gob.prototype.updateOctree = function(){
		...
		let newOz = this.z();
		let newOr = this.r();
		if(newOx !== this.ox || newOy !== this.oy || newOz !== this.oz || newOr !== this.or){
			this.sim.rem(this);
			this.ox = newOx;
			this.oy = newOy;
			this.oz = newOz;
	...
	GB.sim = new GB.Sim('$testNs567'); //a default sim to get started fast
	
	Sim.prototype.newGob = function(up,l,r,optionalP,optionalV){
		return new GB.Gob(this,up,l,r,optionalP,optionalV);
	};
	...
	//param  is TODO which of these?... the string form vs a DV/Dagverse.js (ed25519) pubkey object
	...
	dagball.Gob = function(){
		...
		this.zv = Infinity;
		this.gradient = 0;
		this.lock = false; //TODO gob.lock = true or false, to prevent changing of p and v, like to lock a circ or sphere in a certain x y z r?
