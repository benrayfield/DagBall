...
const AG = (()=>{
	...
	new Op('Vel',1,'(Pos axgob) -> the current mutable velocity of that axgob aka axgob.v, viewed as an Axgob whose b is 64 and whose .i and .j are the 64 bits of that double/float64, or if running in symRule then likely will be compiled to javascript code and run 100million ops/sec.');
	...
	new Op('Kvel',2,'(Kv axgob addToDv) -> 0.0, add to axgob.Kv, see Dp');
	...
	new Op('DoRule',1,'(DoRule query rule) In a given query results (list of gobs or list of [gobA,gobB] etc), do the given rule on those gobs or pairs of gobs. TODO maybe it should always be a [gobA] even if its only 1 gob? In Gob.js the query is called SelFn (select query/function), and rule is called RulFn (rule function), though 2024-3 that code was not finished and I moved on to Axgob.js this file.');
	new Op('InGob',1,'For making single loops in symRule system, for making custom game physics and gameplay rules. (InGob gob) -> [...]. Uses sim.oct/Octree to find gobs whose centers are in a sphere volume. Returns a [] list of axgobs (UPDATE: list of lists of axgobs, and the inner lists list are always size 1) that have an x y z and rad field, which defines a sphere, and are inside the given axgobs sphere, if it also has those fields. TODO This might be optimized as a js [] list or a js Iterator/Generator function* or yield* inside such functions.');
	new Op('NearPairsInGob',1,'For making double loops in symRule system, for making custom game physics and gameplay rules, like within a sphere volume in the game world the Rock Paper Scissors team color capturing experiment I did in videos where colored balls chased eachother, expanding their color exponentially while keeping the total number of balls constant (changing balls color when 2 balls meet depending on their current colors in a rock paper scissors size 3 tournamentGraph). (NearPairsInnGob gob maxDist) -> [[a,b],[a,c],[d,f]...]. Uses sim.oct/Octree to find near pairs of gobs that are both inside the given gob, and each such pair is at most maxDist (a float64) from eachother. TODO This might be optimized as a js [] list or a js Iterator/Generator function* or yield* inside such functions.');
	new Op('GobSearch',1,'A hard to optimize turing-complete query using the Octree and lambdas in general any way you want. (GobSearch nGobsPerResult predicate) -> [...list of [...n gobs...],[...n gobs...],...] where predicate([...n gobs...]).i is nonzero. This op should in theory be able to simulate InGob and NearPairsInGob but is so much more flexible and hard to optimize its for future research. You normally use it in DoRule the same as you use InGob and NearPairsInGob in DoRule.');
	...
	new Op('Field',2,'(Field axgob fieldName) -> the symField version of the given axgob callpaired on fieldName, same as axgob.cp(fieldName).setSym(AG.symField) but likely will be more optimized.');
	...
	//andOr (2) an Axgob, either way representing the same lambda but different ways of optimizing it, and Azblob is not always deduped. Axgob is always perfect deduped.
	...
	const Axblob = AG.Axblob = function(sim, optionalAxgob, optionalBlob, optionalBlobFrom, optionalBlobTo, optionalParent){
		...
		//PrimaryKey of an Axblob is [blobId,blobFrom,blobTo]. IF there is an axgob but no blob, then blobId equals axgob.agId and blobFrom and blobTo are both 0.
	...
	*/
	const Axgob = AG.Axgob = function(sim, o, b, i, j, l, r){
		...
		this.isCbt = false; //FIXME replace this with this.isb() which gets 1 of the bits in this.o and returns it as true/false.
	...
	Axgob.prototype.cp = function(r,optionalI,optionalJ,optionalO,optionalB){
		...
		return this.sim.obijlr(INT(optionalO),INT(optionalB),INT(optionalI),INT(optionalJ),this,r);
	};
	...
	AG.powOf2SizeIntArrayToAxgob = function(ints, optionalFrom, optionalToExcl){
		...
		if(numInts === 2){
			...
			return AG.symPrimzPrimtIsbCurmOpBIJToAxgob(0, 5, AG.primtInt, 1, infiniteCurMore, AG.op.Arr.op, 64, i, j);
		}else{ //numInts > 2
			let l = AG.powOf2SizeIntArrayToAxgob(ints,from,from+numInts/2);
			let r = AG.powOf2SizeIntArrayToAxgob(ints,from+numInts/2,toExcl);
			let bize = l.b+r.b;
			//2**5 is size of int
			return AG.symPrimzPrimtIsbCurmOpBIJLRToAxgob(0, 5, AG.primtInt, 1, infiniteCurMore, AG.op.Arr.op, bize, 0, 0, l, r);
		}
	};
	...
	Sim.protoytype.W = function(wrapMe,wantAxblob){
		if(wrapMe instanceof Float64Array){
			...
			if(wrapMe.length === 0){
				...
				if(wantAxblob){
					...
					//FIXME moved this func to Sim.prototype, so dont call AG.stuff unless sure it exists.
					return AG.powOf2SizeIntArrayToAxgob(powOf2NumberOfInts).setB(wrapMe.length*32); //FIXME this only sets it for the top node but not that.r that.r.r and so on.
				}
			}
	...
	AG.primtAxgobId = 5; //Example: a 256 bit global id of an Axgob, or a 64 bit (actually only using 53 of the bits for the positive integer part) agId/locid of it.
...
let test4Ints = AG.W(Int32Array.of(100,101,102,103));
