...
//
//The core of my new system is: NODE = (int64 position velocity) OR (int64 NODE NODE position velocity),
//where the int64 and NODE are immutable, and position and velocity are mutable.
//This defines a superexponentially sparse dimensional turing complete matrix.
//
...
const AG = (()=>{
	...
	const Op = AG.Op = function(name,cur,description,func,funcI){
		this.op = AG.ops.length; //FIXME this is 256 possible opcodes but theres actually 512 if u count binheap indexing and evaling/0, in o9. This is the low 8 bits.
	...
	Op.prototype.o9 = function(){
		return 256|this.op;
	};
	//returns an Axgob of this op (o9, 512 of them, 256 at 8 curries and the other 256 are prefixes of that or Evaling)
	//in the given Sim. You need new Axgobs for each Sim
	...
	Op.prototype.new = function(sim){
		let o9 = this.o9(); //will always be 256..511 here.
		let U = sim.U;
		let UU = sim.UU; //U.E(U) which in this case equals U.cp(U).
		if(!U || !UU){
			throw new Error('No sim.U or sim.UU');
		}
		let ret = U; //a leaf
		for(let i=7; i>=0; i--){
			ret = ret.E(((o9>>i)&1) ? UU : U); //add 1 opbit
		}
		if(AG.o9IsBit0OrBit1(o9)){
			ret = ret.setBize(1);
			console.log("FIXME if(Math.random()<.1) throw new Error('FIXME set .i for Bit0 and Bit1, considering endian. Also, do i need 2 ints now that its Bit0 and Bit1 recursively and .i and .j ints are caches of that... Will likely be 64k of them cached for all uint16s or will happen on its own sparsely (Axblob is for bigger data).')");
		}
		ret.opName = ret.name = this.name; //local name of this constant/lambda, has no effect on forest shape or 256 bit global id.
		return ret;
		//throw new Error('theres o9, not the (U (U U)) way. just use cp (callpair) to call U on U vs on (U U) 9 times to make each of 256 ops, 511 if you include the binheap indexing, and 0 means evaling.');
		/*
		...
		//old: throw new Error('(U (U U)) is to become Bit0, and ((U U) U) is to become Bit1, and ((U U)(U U)byte) is to become the 256 opcodes, so derive op from that. op and curm and isb are all supposed to be derived. let .cp do it, dont do it directly here.');
		let o = AG.isbCurmOpToO(isb,this.cur,this.op);
		//let b = 0; //bize/bitstringSize
		let b = AG.primtPrimzBizeToB(primt,primz,0);
	...
	//map of o8 to op. thats the low 8 bits of o9. the high bit is always 1 when number of curries is 8 cuz thats where op is first known. before that its binheap indexing.
	const ops = AG.ops = [];
	...
	new Op('C',0,'The number of curries depends on the list length of the first param. (C [funcBody a b c d...] valA valB valC valD...) -> (funcBody (Lz (C [funcBody a b c d...] valA valB valC valD...))), similar to the λ/lambda opcode in Wikibinator203. It can do 1..61 params. 62 is max params that anything can take without taking unlimited/infiniteCurMore params, and the [funcBody a b c d...] is one of those params.');
	...
	/*Going back to Arr, no Bit0 or Bit1. i and j are direct data.
	AG.opBit0 = new Op('Bit0',27,'one bit of a bitstring, which bize27 can say is length 0 or 1 (or if combine more of these up to 2**27-1 bits per bitstring).');
	AG.opBit1 = new Op('Bit1',27,'see Bit0');
	AG.o9Bit0 = AG.opBit0.o9();
	AG.o9Bit1 = AG.opBit1.o9();
	if(AG.o9Bit0&1){
		throw new Error('AG.o9Bit0 must be even so if you ignore the low bit, AG.o9Bit0 and AG.o9Bit1 equal so its faster to check.');
	}*/

	/** Possible ways to use AGI opcode (and make its first param be the name of the experiment so can mount an unlimited
	number of such experiments at once in (AGI 'experiment54335') etc).
	TODO should it enforce that (AGI x) can only be called on params where all paths lead to (AGI x), for any specific x,
	and (AGI y) can only be called on params where all paths lead to (AGI y), etc?
	Use (Field nohalt) (Field haltonself) (Field haltonother) (Field chanceIsS0) (Field chanceIsS1) (Field chanceIsS2) etc,
	between (AGI y)Ay and (Ay (Ay Ay) (Ay Ay) Ay Ay (Ay Ay)) etc. (AGI y)Ay means the name of the constant (AGI y) is Ay.
	All the opcodes defined here would be combos of Ay, not the other axgob opcodes. Only Ay in the Ay experiment.
	 
	 
	this is a different universal combinator than axgob or wikibinator203 and is specialized in PluginHook being like a neuralnet by a bunch of vectors that are each fn having a stochastic vector (sums to 1) of its .red edge to U vs (U Ret).

	make a lambda based AGI by PluginHook being a bayes weight on every possible integer->integer mapping (of positive integers only, actually stored or computed as lambdas).

	im designing a universal pattern calculus combinator of 7 curried params which defines opcodes including Pair (church pair) S T (church true) Lz ((Lz x) is lazyeval of x, a halted view of x whether x is halted or not) PluginHook (this is where neuralnetlike stuff goes, a map of lambda to lambda forall lambda, a func of 1 param). from every lambda theres a red edge that points at U if the lambda does not halt and points at (U Ret) if halts on Ret. L and R are its 2 child lambdas. forall lambda x (L x (R x)) equals x but things get evaled before L and R get childs of them. U is the root/universal lambda. (U U) is it called on itself. (U U U U U U U U) is a call of it on itself 7 curries deep. (U (U U) (U U) U) etc. opcodes are chosen by the first 6 params being U vs anythingExceptU which i call an opbit.

	5: FuncOf5Params
	5: S Pair T F L R IsU PluginHook
	3:
		2: Lz
		2: LzL LzR
	1: LzE //go along red edge from param.
	2: LzLR
	3:
		2: Eq
		2: GodelLessThan

	red edge points at U if doesnt halt or (U Ret) if halts on Ret.

	7 params.

	todo include treemap?

	include prefix that means to interpret as unicode text.

	use pairs of T and F as bitstrings?

	Optimize math on up to int16s, storing them in ints, and dont create the lambdas if its that, just use the int itself? How about int54 as ids which fits in float64? Could fit all the int32s in that.

	Could include all the (int,int)->int math ops.

	I want treemaps and var names in lambdas. var names will be lambdas.

	i want utf8 text.

	I want the first 210066388901 lambdas (all of them up to height 6) included, which fits in 38 bits, except for the cbts. So these 2 kind of literals have some overlap, pic the int1..int32 form of it when so.

	Sucks that only up to height 6 fits but i want 7 params.
	U 0
	(U U) 1
	(U (U U)) 2. so i want height 8 for defining the opcodes.
	Theres 21 lambdas at (not including below) height 3. I could just use those as the opcodes.

	space for 21 opcodes...
	FuncOf5Params
	S
	Pair
	T
	...
	LzL
	LzR
	LzE //go along red edge from param.
	LzLR
	Eq
	GodelLessThan
	Or just use those 16 opcodes and make it start as height 4?
	NO, cuz i want them to all end at same number of curries.
	ill figure something out. this is just an optimizing issue.

	I want high dimensional vectors where red edge from each node is a stochastic vector that points at a set of nodes. Points at U if not halt else at (U Ret).
	Swarm a bunch of these together to converge by some energy function.

	If x is halted, then x.red = (U x).
	If x is not halted then x.red = U.


	Do I want ax? thats vararg but trying to make all funcs take 7 curried params.
	Do I want FPR (func param return) subset of ax but not ax itself?

	Define the energy function....

	Lets start with S which is one of the hardest few opcodes to define energy function for.

	Energy of: (S b c d).red = e.
	What is its energy?

	Start by solving it the forward way:
	(S b c d) = ((b d)(c d)).
	So if ((b d)(c d)) does not halt then energy depends if e is U or not.
	If ((b d)(c d)) halts on bdcd then energy depends if e is (U bdcd) or not.
	If ((b d)(c d)) halts then its lowest energy when (S b c d).red = ((b d)(c d)).red.

	L and R are hard pointers, not stochastic vector like .red edge is.

	In all cases of S of 3 params where all 3 params are halted, energy is lowest when (S b c d).red = ((b d)(c d)).red, and depends only on that so is higher when its not that.

	Everything is a lazyeval and uses red edge to eval statelessly.

	Even if (S b c d) is not halted, its still lowest energy when (S b c d).red = ((b d)(c d)).red but make sure to get b.red.r and check if b.red is U meaning b does not halt and in that case (S b c d).red = U aka the whole thing does not halt.

	How do I define (S b c d).red = ((b d)(c d)).red in a way a js enfunc (javascript energy function) can practically compute?

	Check that (S b c) and d are both halted (no need to check b and c individually for that).

	7 param universal pattern calculus combinator, lazyevaled so can go to unlimited params but everything evals exactly at 7th param, and there is nothing halted with more than 6 params. 

	7:U_else_FuncOf6Params
		5: S Pair T F L R IsU PluginHook
		5: other stuff
			Lz
			LzL
	...
	I am trying to write an energy/loss function for this data structure, where from each lambda there are L and R edges to 2 child lambdas and a HyperEval edge to U if does not halt or (U Ret) if halts on Ret. HyperEval is stochastic vector at any lambdas. energy func is on all that.
	If I write this energy function I will know for certain that solving it to low energy is AGI.
	This would be a convolutional energy function in the space of DAG nodes that each have 2 childs and no data.


	I want to define energy in terms of crossing single edge at a time, so will need something like  wikib_fn_tape. I want to say things like (S x y z).edge5.edge28.edge2.edge1.red etc. I want to write a math expression for the energy as sum of a bunch of those.




	Use fntape (like in wikibinator203 that i didnt finish or only did basic tests or experiments with) on fns at lazy level, meaning i dont care if theyre halted or not, theyre just graph nodes.
	Use it as a 2d matrix, 1d for which fn, and 1d for which fn it points at. L and R are exact but the matrix is for red/HyperEval edges. Dont change the matrix in this fntape calculation. Just compute poten (potentialEnergy) by adding (never subtracting) various times.
	tapeL
	tapeR
	tapeCopy
	tapePaste
	tapeRed
	tapeL //after go along red edge and get R of that to get what it evaled to get L of that. This might be a problem cuz depends on stochasticMatrix content. Maybe I should leave out jumping based on that? Dont know if I can. But I could select from the stochasticVector a possible fn it points at by chance of its weight there, like in boltzmann neuralnets weightedCoinFlip(sigmoid(...)).
	tapeR //like tapeL.
	tapeLazyL //its L child, regardless if its halted or not.
	tapeLazyR //like tapeLazyL.

	Maybe enfunc (the energy function im designing) should take a param of every fn already has a red edge, selected by chance from each stochasticVector (which sum to 1 and are all in range 0 to 1) proportional to that chance.
	So if its observed multiple times during the same computing of enfunc it would give the same answer.
	Yes, do that, but only as needed, weighted observe fn.observeRed from fn.stochasticVectorRed as needed, a different fn.observedRed per call of enfunc.
	Do a bunch of these with different observations.
	Also be careful of how deep it creates new forest nodes / fns during enfunc.


	How does observeRed work during ((b d)(c d)) of (S b c d)? What happens when it creates (b c) and looks for what that evals to along .redObserve.r?
	Do we need to eval it? strange but it seems no, at least not yet. just create the ((b d)(c d)) lazyeval node (fn) and check its redObserve. But where does its redObserve go?

	The definition of halted in this system is simple: .l is halted AND .r is halted AND self is at most height 6.
	Lets put in a number for isHalted, maybe a chance 0 to 1, or should it be a bit? chance depends on where the red edges of other stuff goes.
	isHalted is 0 if this.redObserve===U else is 1.

	I have a SAT solver in rule110convfield. Use a variant of that for various chance vars in fns.

	Think of it as a 2d matrix. fn is on diagonal. At top left is U. In leftmost column if the value in cell is 1 that means red points at U aka not halted.
	From now on, think in terms of this matrix instead of the fn objects with pointers. the fn objects will still exist cuz its superexponentially sparse and to store chance vars in, but think of it as 2d matrix and display it that way sometimes.

	store stochastic vector as 1 chance scalar per node that means the chance node.l.red points at node.r.
	Also maybe there should be a node that represents "outside those observed so far" aka "somewhere else"??? Might cause more problems than its worth.
	Fn is node.

	Each fn has l and r child fns and a small Float32Array or maybe Float64Array of chances to do SAT solving on (like in rule110convfield).

	One of those floats is chanceRed. Also lets have chanceNotRed, and chanceRed+chanceNotRed <= 1 so it can model uncertainty?
	chanceIsU
	chanceIsNotU
	chanceHalted.
	chanceNotHalted???
	...
	Simplify red edge to just point at Ret if halts on Ret else point at U, and have a separate chanceHalts (and maybe also chanceNotHalts, todo rename chanceHalted vs chanceHalts?).

	The main benefit of this system is that PluginHook is a universal function approximator by example and can be called as a lambda.



	Maybe its better to just make the data structure with the Float64Array(floatsPerNode) and add 1 rule at a time which adds to poten. rule(fn)->addToPoten, and keep doing that until it works by gradientDescentWithMomentumAndHarmonySearch.


	Heres the thing... I normally compute these forward and just cache the red edges and dont change them after that. BUT since this is for learning PluginHook lambda by example, such as ((PluginHook "hello") "world").redness==1 && (PluginHook "hello").halts==1 means calling plughk on hello returns world. I should focus on the uncertainty and multiverse of PluginHook being learned by example. That means I dont need a system for doing deep evals alot deeper than 7 curries, lazy stuff in lazy stuff in lazy stuff... I need a system that can do turing complete inference on ((PluginHook "hello") "world") etc.

	fn Red //Ret or U
	chance HaltOnSelf
	chance HaltOnOther
	chance NoHalt
	chance ReadyToForwardCompute //has exactly 7 params and self is a lazyeval and self.l is a HaltOnSelf and self.r is a HaltOnSelf.
	chance DeepLazy //either or both childs have any ReadyToForwardCompute or DeepLazy recursively.

	By poten, hold self.NoHalt >= max(self.l.NoHalt,self.r.NoHalt).

	(S b c d).red === ((b d)(c d)).red; //U if NoHalt or if halts on U.
	(S b c d).NoHalt === ((b d)(c d)).NoHalt;

	.red always points at a HaltOnSelf.

	Generate fns (lazy lambda calls aka nodes) while computing forward, but cant compute (PluginHook x) forward, has to be computed by example. PluginHook is a func of 1 param so theres no deep calculations. It can be any mapping of HaltOnSelf lambda to HaltOnSelf lambda. So when that leads to PluginHook, just pick a lambda at random for each call and let it converge by enfunc based on the examples like ((PluginHook hello) world).redness===1.

	put a few kinds of gas in each node, including:
	* gasTryToProveMe //my red edge and chance vars.
	* gasCreateNodes //to observe the sparse dag, used with gasTryToProveMe since proofs often cost observing more ndoes.
	* gasExpandRedStochasticVector //if red edge is stored in a variable size stochastic vector per node (instead of using parent nodes as key/val pairs of red edge) then need this to expand it.

	This is gonna work cuz i can compute forward on ReadyToForwardCompue and DeepLazy fns (other than PluginHook) creating nodes along the way to compute by sparse NSAT as scalarField, but im gonna have to reorganize the stack to work mostly timelessly with the only timeful part being the movement of gas (various kinds of gas) between the nodes.
	...
	I need some way for a node to get back the gas it gave to another node if the other node finishes its calculation with gas left over, or else the gas is gonna drain in many directions quickly and not be able to calculate much.

	Make the red vector be a js {} of locid to locid. Locid is a local unique id of a fn. Ill just make it a small string for now. see dedup hashtable in axgob and wikibinator203. So even if the fn is garbcoled it only leaves a map of string to string, which todo should also be removed?
	Or if I use a 43 char id (base64 of 256 bits) then its true regardless of if its in memory or not. Leave that as an option, but use uint53 ids for now.
	have a js {} of id to fn shared by all fns to look that up in. Call it sim.idToFn. rename sim to vm?

	Also have a fn.cp = js {}, a map of x to (this x) for any lambda x, for dedup.

	make fn.toString return its locid so can do fnX.cp[fnY]. and make value in .cp be a fn, not a string. key has to be string in the basic kind of js {} map. js Map objects are alot slower.

	red edge will be someFn.red = {} where key is fn.toString() and val is number that tries but doesnt always succeed at staying in range 0 to 1 cuz is by enfunc.


	I want the ability to have multiple whole system states, each as a vector, varying the number vars per fn and the red edge numbers. I want to harmonySeach these.
	So fn.nums and fn.red could for example be fn.forks[forkId].nums and fn.forks[forkId].red. Would that be too slow? What if its just fn[forkId].nums and fn[forkId].red?
	fn['l'] aka fn.l would still be its left child, and other fields.
	Make Fork object that contains those forkId details, one Fork per forkId. Each is a superexponentially sparse turing complete vector and can be mutable or frozen to be immutable.
	Make fn be a js [] so it automatically gets a length? Or make it be a {} and use string forkIds for sparseness?

	Define enfunc(fn,forkId) which can be wrapped and looped in Enfunc(Fork). Total energy aka poten is sum of enfunc(fn,forkId) forall fn in that forkId.
	...
	I need some way for a node to get back the gas it gave to another node if the other node finishes its calculation with gas left over, or else the gas is gonna drain in many directions quickly and not be able to calculate much.
	...
	SOLUTION: make a paidYouRecently js {} parallel to the .red {}, which will decay slowly, and if there is various kinds of gas left they get refunded up to that amount when its finished converging to a proof of itself. Will need reverse edges.
	...
	Turn such edges into an Edge object so both sides of the edge can have a pointer to it, and multiple numbers can go in an Edge, including red and 1 paidYouRecently for each gas type to track.

	Make a separate fn for each fork of its forestShape, and 1 fn as the mutable set of fns of the same forestShape, so fn.forks[forkId] is a fn of the same forest shape, and each of them should have fn.l and fn.r pointing at the fn in its own forkId.

	Do I need observedRed decided in all fns (of the same forkId) before calling enfunc(fn,forkId) on all of them? Aka the energy of a specific set of func_param_return not the matrix form of .4 chance of returning b and .6 chance of returning c etc.
	If so then maybe its best for Fork to be immutable and to have a form of it that observedRed is decided and another form that .red is a {} of fn to chance.
	For each such fork, there is a potentialEnergy_aka_loss that is the sum of enfunc(fn,forkId) forall fn in that forkId. So graph evolution algorithms and harmonySearch and gradientDescentWithMomentum could all be used in various combos together.
	Each fork is a sparse 3d matrix, 2d being sparse of from/to each pair of lambdas, and 1d being how many number vars per edge, and also has a 2d matrix of numLambdas*numberVarsPerLambda for vars like NoHalt HaltOnSelf HaltOnOther IsU etc.
	So a fork is a pair of superexponentially sparse tensors.

	Define the integer 0 as "unknown" which is used in fn.red[0]=1 (or maybe fn.red.unknown) to say dont know which fn this evals to or if this halts or not.
	What happens if you try to eval (x y) but (x y).red.unknown is 1 (so (x y).red.otherForkId=0)? It would have to make it known. Move some kind of gas to (x y) so it tries to use that gas recursively to find where its red edge points and if itself is NoHalt HaltOnSelf or HaltOnOther and generally prove itself.

	Change it to fn.forks[forkId] instead of fn[forkId], so its easier to loop over them. forkId is any string you choose.


	enfunc has to work on observedRed pointers observed from chances in (fn.red[x]. fn.red[x]+fn.red.unknown)===1 IFF fn evals to x.

	I can in theory efficiently fork sets of random 256 bit numbers by using their xor as the id of the set but it only works if someone is not trying to find a collision of 2 different sets that have the same id. It very probably wont happen by accident.
	Each possible set has up to numNodes possible edges, to include/exclude an int256 from the set, to flip the true/false of does the set contain it.
	But to prove the id of the set you still have to hash all its contents (which is faster than the other way of xoring the hash of all pairs of int256s in the set), so this is just a localToAComputer optimization.
	Enfunc(int256) would be the potentialEnergy_aka_loss of that [set of NumbersInANode and NumbersInAnEdge].
	There must be at least 1 path of int256s connecting all int256s that are in memory to eachother, and often alot more than 1.
	...
	·
	Now
	For this so far nonexistant(though far away aliens might have them?) kind of AGI, Im thinking of making a superexponentially sparse tensor that can for example do a 30googolplex X 200googolplex X 5 X 29 X 27googol address space with a number in each cell, not storing the 0s.
	*/
	//new Op('AGI',infiniteCurMore,'an Infcur-like place to mount AGI experiments. TODO. [I put an opcode into Axgob (my newest universal combinator half working so far) new Op(\'AGI\', that is just a list with no size limit, to mount such AGI experiments in, by defining game rules that act on number vars (like x y z rad red green blue manaRefillRate etc) attached to it]');
	new Op('Ls',infiniteCurMore,'Renamed AGI op to this Ls. Mount other experimental combinators and other lists of stuff here, to be customized using (Field manaRefillRate (Ls name5435 stuff...)).p .v etc. (Edge edgeType55 lsA lsB).p. Use first param as name/type such as "experiment634534". (Ls experiment634534)Ex34 then (Ex34 blah blah stuff blah). an Infcur-like place to mount AGI experiments. TODO. [I put an opcode into Axgob (my newest universal combinator half working so far) new Op(\'AGI\', that is just a list with no size limit, to mount such AGI experiments in, by defining game rules that act on number vars (like x y z rad red green blue manaRefillRate etc) attached to it]');
	//TODO rename AGI to Namedlist Typedlist or something, since that could be more general than just attempted AGI experiments. (Namedlist agiExperiment53342435) for example.

	//see sim.U instead: new Op('U',infiniteCurMore,'basically a constant, doesnt do anything, but need to choose one for (F U) to be the L of every leaf.');
	...
	//OLD, sym isnt in the o int anymore: START axgob.o

	//new Op('Sym',1,'(Sym axgob) -> int in range 0..255. Theres a few things that go in axgob.o int.');
	//new Op('SetSym',1,'(SetSym axgob int) -> forkEdited axgob so that (Sym (SetSym axgob x)) -> x, but x is ANDed into range 0..255. Theres a few things that go in axgob.o int.');
	new Op('Primz',1,'like Sym and SetSym');
	new Op('SetPrimz',2,'like Sym and SetSym');
	new Op('Primt',1,'like Sym and SetSym');
	new Op('SetPrimt',2,'like Sym and SetSym');
	new Op('Isb',1,'like Sym and SetSym');
	new Op('SetIsb',2,'like Sym and SetSym');
	new Op('Curm',1,'get number of curries more before eval, or 63 means infiniteCurMore. Range 1..63, since if curm is 0 then this wont eval on it until that evals (to something whose curm is positive).');
	new Op('Op',1,'like Sym');
	new Op('SetOp',2,'Sets o8/op all the way recursively down to a leaf along this.l.l.l.l.l.l.... and if that leaves this as evaling then this may be halting or nonhalting, have to eval right away. Remember everything is lazyEval but following axgob.e or axgob.ee.');
	//
	//new Op('Sym0',1,'(Sym0 axgob) -> the symNeutral form of axgob. Example: (Sym0 (Rule axgob)) -> form of axgob thats not a Rule, Field, etc.');
	
	new Op('Field',3,'(Field axgob fieldName) -> the symField version of the given axgob callpaired on fieldName, same as axgob.cp(fieldName).setSym(AG.symField) but likely will be more optimized. The most common 4 fields are x y z rad (defines a 3d sphere volume) in the game space, normally marked by (Dpos (Field someGob2233 "rad")) being a positive radius (is 0 by default cuz .p and .v are 0 by default), and similar for "x", "y", and "z", which likely ill make a syntax for that tokens that start with a lowercase letter and have no whitespace are a string literal so (Dpos (Field someGob2233 rad)) with no quotes, and likely some more complicated syntax you can just write it as someGob2233.rad.x if you want to see the x position of the radius of someGob2233 but normally you would only go up to 1 level deep someGob2233.x or someGob2233.rad but not view its radius as an object in the game world too, but its important to have self awareness self reference pattern-calculus etc to think about thoughts and compute about computing etc.');

	new Op('Edge',4,'See (Edge edgeType55 lsA lsB).p in Ls op comment');

	new Op('Rule',2,'(Rule axgob) -> same as axgob.setSym(AG.symRule) but likely will be more optimized. rule.p is like dagball.Circ.influence in that its 1 to include a rule and 0 to exclude that rule (default p and v are 0) and gradually can be any finite float64 value. Rules can therefore turn on/off other rules or vary their strength. A gob, like a team colored ball in the Rock Paper Scissors team color capturing experiments (see videos), also has a .p used like .influence, that if such balls .p is 0 it means to not include that ball in physics calculations, and if 1 then include it.');


	//OLD, sym isnt in the o int anymore: END axgob.o

	//START axgob.b
	new Op('B',1,'Get bize/bitstringSize which ranges 0 to 2**30, which is stored in axgob.b int.');
	new Op('SetB',2,'Set what (B axgob) returns but is ANDed into the range allowed by that or the max bits that fit in the leafs reachable from here (axgob.i and axgob.j are 64 bits of leaf).');
	//START axgob.b

	//START axgob.i and axgob.j which you can put whatever 64 bits in you want
	...
	//END axgob.i and axgob.j which you can put whatever 64 bits in you want
	...
	console.log('TODO AVL tree, GoO, OO, (O?), getter and setter syntax, as in what GetState, SetState, If, Ife, and Wh use, etc?');
	...
	new Op('LzI',1,'FIXME make cbt/bitstring/i/j depend on forest shape only, using Bit0 and Bit1 opcodes. Lz means lazyEval. (LzI (Lazy x)) -> x.i.');
	new Op('LzII',1,'FIXME make cbt/bitstring/i/j depend on forest shape only, using Bit0 and Bit1 opcodes. Lz means lazyEval. (LzII (Lz x)) -> (Lazy x.i), despite that x.i cant be a lazyeval cuz is an int but to stay consistent with LazyL and LazyR it wraps it in a Lazy.');
	...
	/*sym* has been replaced by ops, like (Field manaRefillRate thing) and (Rule thing).
	...
	*/
	...
	/** This is a universal-pattern-calculus-combinator with perfect dedup nomatter how its called on itself to make more combinators.
	Use Axblob for bigger faster data such as wrapping Float32Arrays in CPU and GPU memory with lazy-dedup.
	Axblob wraps a primitive array andOr an axgob of the same data.
	Axblob should todo go up to a few teraflop/sec in good gaming computer,
	and as axblob in theory a few million lambdas/sec (wikibinator203 does 400k lambdas/sec but is more complex than axgob).
	2024-3-29+ designing this way, todo...
	DIRECT DATA: b i j l r.
	CACHE: o.
	o, b, i, and j are ints. o is a cache derived from forest shape only (TODO remove it from constructor here?).
	i and j can be anything, 64 bits of literal data, but only bize amount of it is used. Bize is low 27 bits of b.
	High 2 bits of b are primt2. Next lower 3 bits of b are primz3. Max bitstring size is 2**27-2 bits, and 2**27-1 means is bigger or is not a bitstring.
	The normed form of bitstrings uses Arr opcode, which using primt can be other, int, float, or unicode,
	of size 2**primz per primitive such as double, byte, short, int, float, etc.
	l and r are child Axgobs. If it is a leaf (and there are nearly 2**32 possible leafs, the allowed vals of b int of that leaf)
	then l is (F U) aka identityFunc and r is thatLeaf. The normed leaf has b of 0 and is called U.
	*/
	//const Axgob = AG.Axgob = function(sim, o, b, i, j, l, r){
	const Axgob = AG.Axgob = function(sim, l, r, b, i, j){
		//let b = optionalB|0;
		//let i = optionalI|0;
		//let j = optionalJ|0;
		...
		//Immutable universal-pattern-calculus-combinator stuff (o b i j l r). UPDATE: o is cache, not direct data.
		...
		if(this.h){ //nonleaf
			if(!l){
				throw new Error('No l. Maybe its still booting so hasnt closed the quine loop yet where L of U is (F U) and R of U is U. or maybe some other bug.');
			}
			let lCurm = l.curm();

			let isEvaling = lCurm <= 1; //FIXME, in some cases that just means to check it again to see if can raise curm, like in Ax op.
			//let h = 1+Math.max(this.h, r.h);
			//TODO set o9 of every leaf to 1.
			//let lO9 = this.o9();
			let lO9 = l.o9();
			let rO9 = r.o9();


			//let o9 = isEvaling ? 0 : (lO9&256 ? lO9 : ((lO9<<1)|(r.isU()?0:1)));
			let o9 = isEvaling ? 0 : (lO9>=256 ? lO9 : ((lO9<<1)|(r.isU()?0:1)));
			let isb = AG.o9IsBit0OrBit1AsInt(o9)|(this.isb()&r.isb()); //both are bitstrings. FIXME make Bit0 and Bit1 be bitstrings

			let curm;
			//if(!l || ){
			//	curm = 8; //8 params until o9 opcode is known. l is null when
			//}
			if(!lCurm || !r.curm()){
				curm = 0; //Evaling
			}else if(lCurm === infiniteCurMore){
				curm = infiniteCurMore;
			}else{
				curm = lCurm-1;
			}
			this.o = AG.isbCurmOpToO(isb,curm,o9);
		}else{ //leaf
			let isb = 0;
			let curm = 8; //8 params until o9 opcode is known. l is null when
			let o9 = 1; //binheap indexing starts as 1. o9 of 256 to 511 are the 256 main opcodes. o9 of 0 means evaling.
			this.o = AG.isbCurmOpToO(isb,curm,o9);
		}
		...
		//throw new Error('TODO derive int o cache instead of taking it as constructor param');
		//let isb1 = TODO;
		//let curm6 = TODO;
		//let o9 = TODO;
		//this.o = AG.isbCurmOpToO(isb1, curm6, o9);
		...
		if(!this.l){
			...
			console.log('TODO remove this test after bootCloseTheQuineLoop is working');
			if(this.h > 0 && this.l.h !== this.h-1){
				...
				console.log('FIXME??... this.l.h='+this.l.h+' but this.h='+this.h+' which must be either 1 bigger or maxint to mean unlimited');
			}
		}
	...
	let Sim = AG.Sim = function(namespaceString){
		...
		let uo = AG.isbCurmOpToO(0,9,1);
		let ub = 0; //AG.primtPrimzBizeToB(AG.primtOther, 0, 0);
		let ui = 0, uj = 0;
		//this.U = this.obij(uo,ub,ui,uj);
		this.U = this.bij(ub,ui,uj);
		//fill in this.U.l after creating this.Ide after the loop of 256 opcodes below that includes F so can derive Ide/identityFunc as (F U).
		this.U.r = this.U;
		this.UU = this.U.cp(this.U);
		/*
		//FIXME this is likely to cause infiniteloops until the quine loop is closed the right way, will take some experimenting like it did in wikibinator203.
		...
		*/
		this.op = {};
		for(let op of AG.ops){
			this.op[op.name] = op.new(this); //calls cp on combos of sim.U and sim.UU. create S T Pair Ax L R Bit0 Bit1 etc, all 256 of them in o9 range 256 to 511.
			console.log('sim.op.'+op.name+' = '+this.op[op.name]);
		}
		this.Ide = this.op.F.cp(this.U); //identityFunc
		this.U.l = this.Ide; //Forall x, (L x (R x)) equals x.
		...
		this.F = AG.op.F.new(this);
		console.log('Sim.F = '+this.F.toStringC());
		//changed in redesign 2024-2. F is not a leaf. AG.testEqq(this.F.r, this.F, 'F. leaf starts with .r as itself, and during boot, .l is filled in as Ide/identityFunc soon');
		//changed in redesign 2024-2. F is not a leaf. AG.testEqq(this.U.h, 0, 'F. Every leaf height is 0.');
		//this.Pair = this.i(AG.op.Pair.op);
		//this.S = this.i(AG.op.S.op);
		//identityFunc
		this.Ide = this.F.cp(this.U);
		...
		AG.testEqq(this.Ide.r, this.U, 'Ide.r is U');
		//AG.testEqq(this.Ide.h, 1, 'Ide as (F U), height should be 1.');
		AG.testEqq(this.Ide.h, 10, 'Ide as (F U), height should be 10 cuz o9 is 8 params of U (each as U or (U U)) and if first of those is (U U) then F is height 9, so (F U) is height 10, else 1 less depending which o9 (still moving ops around 2024-3).');
		AG.testEqq(this.F.h, 9, 'F height is 9');
		AG.testEqq(this.F.l.h, 8, 'F.l height is 8');
		AG.testEqq(this.F.l.l.h, 7, 'F.l.l height is 7');
		//bootCloseTheQuineLoop
		console.log('Setting this.U.l = this.F.l = this.Ide.l = this.Ide');
		this.U.l = this.F.l = this.Ide.l = this.Ide; //the leaf.l's are the quine loop problem, hopefully solved here (TODO keep testing 2024-3-8-5pET+)
		console.log('2Sim.U = '+this.U.toStringC());
		console.log('2Sim.F = '+this.F.toStringC());
		console.log('2Sim.Ide = '+this.Ide.toStringC());
		//changed in redesign 2024-2. F is not a leaf. AG.testEqq(this.U.l, this.Ide, 'U. L of every leaf is Ide');
		//AG.testEqq(this.F.l, this.Ide, 'F. L of every leaf is Ide');
		AG.testEqq(this.U.r, this.U, 'U. AGAIN: leaf starts with .r as itself, and during boot, .l is filled in as Ide/identityFunc soon');
		//AG.testEqq(this.F.r, this.F, 'F. AGAIN: leaf starts with .r as itself, and during boot, .l is filled in as Ide/identityFunc soon');
		//all the .r of leafs are already those same leafs cuz of this code in Axgob constructor: this.r = r || this;
	...
	Axgob.prototype.OpName = function(){
		...
		let o9 = this.o9();
		return o9>=256 ? AG.ops[o9&0xff].name : 'Op'+o9;
	...
	//AG.tostringReturnsAxgobOpName = false;
	AG.tostringReturnsAxgobOpName = true;
	...
	Axgob.prototype.toStringB = function(){
		if(this.opName && AG.tostringReturnsAxgobOpName){
			return this.opName;
		}//TODO return this.name if this is the second or later time the name has displayed in the same tostring (expand on first display, not on later displays).
		let s = 'B';
	...
	/*Axgob.prototype.bootIsb = function(){
		AG.oToIsb = o=>((o>>14)&1);
		return AG.isb
	};*/

	//The b int is primt2 primz3 bize27
	const maxBize = AG.maxBize = 2**27-1;
	...
	Axgob.prototype.cp = function(r,optionalI,optionalJ,optionalO,optionalB){
		...
		//TODO implment (U(UU)) as Bit0, ((UU)U) as Bit1, (UU UU byte params...) as opcode. byte is made of Bit0 and Bit1. limit cbt to cbtHeight that fits in bize27 or maybe 0 to 2**26.
		...
		//throw new Error('TODO use the new AG funcs for making o and b ints. b now has primt primz and bize and is user accessible, vs o and height etc are purely cache of forest shape.');

		/*TODO must have a way to check for U, (U U), (U (U U)), ((U U) U), and ((U U)(U U)), which dont have a byte opcode yet cuz thats the next param.
		or do I want to go back to opbit aka 9 of U vs (U U) as first 9 params of U to choose 2**8 opcodes.
		The 9 way adds duplicates cuz its U vs anything except U (such as (U U)). The (U (U U)) way makes the first 5 lambdas not have an op byte directly.
		Its fast to check this.h being 0 (U) or 1 (U U) or 2 (such as (U (U U)) or 3 opcode byte goes here if prefix is ((U U)(U U)),
		but do I really want to use a design where op byte is not copied from L child in more cases (normally its only in U that it doesnt)?
		Im leaning toward o9. curm is 6 bits so thats 15 bits together. I could go up to o10 and have up to 512 opcodes, but 256 is probably a good number.
		Also, in wikibinator203, sometimes it would have been useful to know the opbits (U vs anythingExceptU) of the first n params, not just those that choose the opcodes,
		but im going with just 9 for now at least. Adding more later would be just a cache if so, not changing opcodes.
		*

		/*
		//AG.o9Bit0 = AG.opBit0.o9();
		//AG.o9Bit1 = AG.opBit1.o9();
		fixme use these to compute isb
		use AG.o9IsBit0OrBit1(l09) andOr rO9 andOr the merged o9 but merging them is slower than doing both of those.
		maybe should only check if neither this/l nor r are bitstring but their combo is (lO9 is about to be one nomatter what r is).
		maybe should only count it as isb/isbitstring if it made only of the normed forms of Bit0 and Bit1?
		Check ==this.sim.op.Bit0 and ==this.sim.op.Bit1 for the normed form, but make sure it exists first,
			so shortcircuit this in some variant of "let isb = this.isb() & r.isb()" so it only happens if height is higher than Bit0 and Bit1 or somewhere around that?

		need to detect the creation of Bit0 and Bit1 here by pairing this and r, only the normed.
		Do I want to cache a bit into o, that tracks if its a normed form of everything leading up to the opcodes, up to the o9?
		*

		let o9 = isEvaling ? 0 : (lO9&256 ? lO9 : ((lO9<<1)|(r.isU()?0:1)));
		//FIXME set isb in Bit0 and Bit1 ops (in Op.new(sim) or cp?).
		//let isb = this.isb() && r.isb() && this.h===r.h; //both are bitstrings and are same height.
		//let isb = this.isb() & r.isb(); //both are bitstrings. FIXME make Bit0 and Bit1 be bitstrings
		//
		//FIXME isb should only be 1 (else 0) if the Bit0 and Bit1 its made of (many of them) are all the normed form of Bit0 and Bit1,
		//but can still have many possible bize sizes and primt and primz.
		let isb = AG.o9IsBit0OrBit1AsInt(o9)|(this.isb()&r.isb()); //both are bitstrings. FIXME make Bit0 and Bit1 be bitstrings
		...
		let o = AG.isbCurmOpToO(isb,curm,o9);
		*/

		let primt = l.primt();
		let primz = l.primz();
		let bize = l.bize()+r.bize();
		if(bize > AG.maxBize){
			throw new Error('TODO define it as infinite looping, not isb, etc?');
		}
		let b = AG.primtPrimzBizeToB(primt,primz,bize);
		let i = optionalI|0; //FIXME endian?, make this be the first 32 bits of this as a bitstring, 0s after that if its smaller.
		let j = optionalJ|0; //FIXME endian?, second 32 bits of a bitstring.
		//return this.sim.obijlr(o,b,i,j,l,r);
		return this.sim.lrbij(l,r,b,i,j);
	...
	//primt2 (then primz3 which chooses primitive size from 2**0 to 2**7, then bize27).
	AG.primtOther = 0; //general bits
	AG.primtInt = 1;
	AG.primtFloat = 2;
	AG.primtUnicode = 3; //UTF8, UTF16, or UTF32.
	...
	*/
	...
	//AG.isbCurmOpToO = (isb1,curm6,op8)=>(((isb1&1)<<14) | ((curm7&0x3f)<<8) | (op8&0xff));
	AG.isbCurmOpToO = (isb1,curm6,op9)=>(((isb1&1)<<15) | ((curm6&0x3f)<<9) | (op9&0x1ff));

	AG.o9IsBit0OrBit1 = o9=>((o9|1)===AG.o9Bit1);
	AG.o9IsBit0OrBit1AsInt = o9=>(AG.o9IsBit0OrBit1(o9) ? 1 : 0);

	AG.primtPrimzBizeToB = (primt2,primz3,bize27)=>(((primt2&3)<<30) | ((primz3&7)<<27) | (bize27&0x07ffffff));

	//OLD, use AG.isbCurmOpToO instead.
	...
	*/
	...
	Axgob.prototype.o9 = function(){ //FIXME rename this to op after axgob.op is replaced by 8 of the bits in axgob.o.
		return AG.oToO9(this.o);
	};
	...
	AG.bToPrimz = b=>((b>>>27)&7);

	AG.bToBize = b=>(b&0x07ffffff);

	AG.primtPrimzBizeToB = (primt,primz,bize)=>(((primt&3)<<30) | ((primz&7)<<27) | (bize&0x07ffffff));
	...
	//it goes in b, not o. AG.oToPrimt = o=>((o>>>16)&0xf);
	...
	Axgob.prototype.primz = function(){
		...
		return AG.bToPrimz(this.b);
	};

	//bize27 bits in b
	Axgob.prototype.bize = function(){
		return AG.bToBize(this.b);
	};

	/*move sym to be an op: (Field x) Rule etc.
	AG.oToSym = o=>(o>>>24);
	...
	Axgob.prototype.setB = function(int){
		...
		return this.sim.lrbij(this.l,this.r,INT(int),this.i,this.j);
	};

	//bize is 27 bit bitstring size from 0 to 2**27-1, describing a tree of concat of bitstrings where all paths lead to Bit0 or Bit1 which each have bize 1.
	//User can set bize to lower than number of bits available but not higher, and must be a multiple of 2**primz such as 32 for int,
	//but normally this happens automatically in cp/callpair of 2 lambdas by adding their 2 bizes and viewing them as concat of 2 bitstrings.
	//TODO what happens when that exceeds 2**27-1?
	Axgob.prototype.setBize = function(bize27){
		return this.setB((this.b&0xf8000000) | (bize27&0x07ffffff));
	};
	...
	Axgob.prototype.setI = function(int){
		...
		return this.sim.lrbij(this.l,this.r,this.b,INT(int),this.j);
	};
	...
	Axgob.prototype.setJ = function(int){
		...
		return this.sim.lrbij(this.l,this.r,this.b,this.i,INT(int));
	};
	...
	Axgob.prototype.setL = function(l){
		...
		return this.sim.lrbij(l,this.r,this.b,this.i,this.i);
	};
	...
	Axgob.prototype.setR = function(r){
		...
		return this.sim.lrbij(this.l,r,this.b,this.i,this.i);
	};
	...
	Sim.prototype.slowDedupKey = function(l,r,b,i,j){
		let s = 'B'+b+'I'+i+'J'+j;
		return l ? ('L'+l.agId+'R'+r.agId+s) : s; //if theres no l, its a leaf/u so just the 4 ints.
	};
	...
	Sim.prototype.lrbij = function(l,r,b,i,j){
		let slowKey = this.slowDedupKey(l,r,b,i,j);
		return this.slowDedup[slowKey] || (this.slowDedup[slowKey] = new Axgob(this,l,r,b,i,j));
	};

	Sim.prototype.lrtzbij = function(l,r,primt2,primz3,bize27,i32,j32){
		return this.lrbij(l,r,AG.primtPrimzBizeToB(primt2,primz3,bize27),i32,j32);
	};

	Sim.prototype.bij = function(b,i,j){
		return this.lrbij(null,null,b,i,j);
	};

	Sim.prototype.tzbij = function(primt2,primz3,bize27,i32,j32){
		return this.lrtzbij(null,null,primt2,primz3,bize27,i32,j32);
	};
	...
	Sim.prototype.bit = function(uint1){
		//return uint1 ? this.op.Bit1 : this.op.Bit0;
		return this.i(uint1&1, 1);
	};

	Sim.prototype.twoBits = function(uint2){
		//FIXME endian?
		//return this.bit(uint2>>>1).cp(this.bit(uint2&1));
		return this.i(uint2&3, 2);
	};


	Sim.prototype.fourBits = function(uint4){
		//FIXME endian?
		//return this.twoBits(uint4>>>2).cp(this.twoBits(uint4&3));
		return this.i(uint4&0xf, 4);
	};

	Sim.prototype.byte = function(byte){
		//FIXME endian?
		//return this.fourBits(byte>>>4).cp(this.fourBits(byte&0xf));
		return this.i(byte&0xff, 8);
	};

	//uint16
	Sim.prototype.short = function(short){
		//FIXME endian?
		//return this.byte(short>>>8).cp(this.byte(short&0xff));
		return this.i(short&0xffff, 16);
	};

	//int32
	Sim.prototype.int = function(i){
		return this.i(i, AG.primtInt, 5, 32);
	};

	Sim.prototype.float = function(float){
		throw new Error('TODO use this.tzbij(AG.primtFloat,5,32,float32Bits(float),0);')
	};
	
	Sim.prototype.double = function(double){
		throw new Error('TODO use this.tzbij(AG.primtFloat,6,64,32bits,other32Bits_fixmeWhichEndian);')
	};

	//2 int32s, like sim.long does int64 of same bize.
	Sim.prototype.twoInts = function(i,j){
		return this.ij(i, j, AG.primtInt, 5, 64);
	};

	Sim.prototype.fourInts = function(b,c,d,e){
		return this.twoInts(b,c).cp(this.twoInts(d,e));
	};

	//could use this for a 256 bit global id. or might want 32 bytes or 256 bits.
	Sim.prototype.eightInts = function(b,c,d,e,f,g,h,i){
		return this.fourInts(b,c,d,e).cp(this.fourInts(f,g,h,i));
	};

	//int64
	Sim.prototype.long = function(i,j){
		return this.ij(i, j, AG.primtInt, 6, 64);
	};

	Sim.prototype.ij = function(i, j, primt2, primz3, bize){
		return this.tzbij(primt2,primz3,bize,i,j);
	};
	
	//returns the wrapper of the int32, one of the 2**32 possible leafs. sim.i(x)===sim.i(x) is true for all int32 x.
	//bize ranges 0 to 32 since only 1 int is given. FIXME endian?
	Sim.prototype.i = function(i, primt2, primz3, bize){
		return this.tzbij(primt2,primz3,bize,i,0);

		//let o = AG.isbCurmOpToO(isb1, curm6, o9);

		//throw new Error('TODO');
		//if(bize > 32) throw new Error('bize='+bize+' > 32');
		//return this.tzbij(AG.primtInt,5,bize,i,0); //FIXME should it be primtInt?
		//return this.bij();
		//return this.obij(o,b,i,j);

		//FIXME endian?
		//return this.short(i>>>16).cp(this.short(i&0xffff));
		/*
		...
		let o = AG.isbCurmOpToO(1, infiniteCurMore, AG.op.Arr.op);
		//let b = 32; //number of bits in the int[1]
		let b = AG.primtPrimzBizeToB(AG.primtInt,5,32); //2**5===32
	...
	Sim.prototype.add = function(axgob){
		...
		this.lrbij(axgob.l,axgob.r,axgob.b,axgob.i,axgob.j);
		if(axgob.isGob()){
			this.oct.add(gob);
		}
	};
	...
	*/
	...
	const Evv = code=>{
		throw new Error('TODO ()=> lazyEval Axgob code string');
	};
	...
	//TODO AG.xt('basic bit math test 1', Evv('(| 1 2)'), Evv('3'));
	//TODO AG.xt('basic bit math test 2', Evv('(<< 3 5)'), Evv('96'));
	//TODO AG.xt('basic bit math test 3', Evv('(& 254 439)'), Evv('182'));
	//TODO AG.xt('add', Evv('(+ 200 10000)'), Evv('10200'));
...
//throw new Error('doIWantToKeepBit0AndBit1VsIJBeingDirectDataCuzItsAFewTimesMoreNodes No, I want to go back to i and j (and still b which has primt primz and bize) being direct data, but o being derived from forest shape, UNLESS it would cause some problem. Think thru it. What problems would it cause? ... makes harmonysearch harder. cuz have to cache 64k axgobs for all uint16s but can sparsely cp them into int32 int64 int128 and so on. Given 64k axgob cache, it only has half as much storage on average cuz pairs 2 uint16s but still chooses any int32 it wants, vs having i and j (64 bits) per node if i and j are direct data not just caches. If i and j are direct data then it takes 1 node to wrap a double/float64, else takes 3 nodes with the shared 64k cached axgobs. float64s are usually stored as just float64 and only converted to axgob as needed. For bigger data use Axblob which wraps js primitive array andOr wraps Axgob. Consider which would be easier for gpujsRecurrentLstmQlearningOfNPCBuildingLambdasIn_getBasicFunStuffWorkingInDagballAxgob to learn. I redesigned to Bit0 and Bit1 to keep it simple, but maybe I sacrificed too much. Consider primt2 primz3 bize27 (in axgob.b) are much simpler and maybe easier to limit to the actual bitstring size if Bit0 and Bit1 exist as ops. Consider that it slows things down to have lazyEvaled .l and .r pointers and they are all dense right now, though I could make it use .L() and .R() and just store data in .i and .j anyways for normed bitstrings at most 64 bits.');
//AG.testEqq(AG.sim.op.Bit0.bize(), 1, 'Bit0 is 1 bit');
//AG.testEqq(AG.sim.op.Bit1.bize(), 1, 'Bit1 is 1 bit');
//AG.testEqq(AG.sim.op.Bit1.cp(AG.sim.op.Bit1).bize(), 2, 'Bit1 called on Bit1 is 2 bits');
for(let i=0; i<10; i++){
	AG.testEqq(AG.sim.i(i).i, i, 'wrap int '+i);	
}
AG.testEqq(AG.sim.i(100).i, 100, 'wrap int 100');
AG.testEqq(AG.sim.i(1000).i, 1000, 'wrap int 1000');
AG.testEqq(AG.sim.i(1000000).i, 1000000, 'wrap int 1000000');
AG.testEqq(AG.sim.i(-3).i, -3, 'wrap int -3');
...
//console.error('skipping Axgob tests cuz its incomplete. TODO Axgob upgrade of dagball after ppl are playing the game together online and the level editor works well for them.');
console.warn('skipping Axgob tests cuz its incomplete. TODO Axgob upgrade of dagball after ppl are playing the game together online and the level editor works well for them.');
