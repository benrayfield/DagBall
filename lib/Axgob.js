...
const AG = (()=>{
	...
	for(let i=0; i<64; i++){
		throw new Error("TODO generate those 64 before the calls below");
	}
	...
	new Op('HasAx',1,'(HasAx thing) -> 1 as true or 0 as false for does thing contain a call of Ax opcode, but Ax by itself does not count, only if it has enuf params (is that 1 or 2 params?) that it has to be verfied. Normally verifying may take up to any finite time to prove a true statement is true, and can never prove a false statement is true. Halts means true in this case. If HasAx is false then its easier to verify. Remember that every forest node is valid whether it halts or not, but (Ax funcBody ...params...) evals to itself if true (cuz funcBody (Lz (Ax funcBody ...params...)) evals to U to mean halts on itself else evals to (U returnVal) to halt on returnVal, else such a call of Ax may infinite loop during verifying in which case the Ax correctly infinite loops (evalsto meaning go along red edge aka evalsto edge aka axgob.e which starts null and is filled in if it evals to something). Does not count (Lazy x) where (HasAx x).');
