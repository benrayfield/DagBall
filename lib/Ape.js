//TODO Ape.Ape is confusing (renamed from dagball.Ape) cuz when someone says ape.something
//do they mean Ape.something or instance of Ape.Ap dot something?
//Choose a different name like Ape.Ap or something.
...
const Ape = {
	...
	website: 'TODO put in https://github.com/benrayfield/DagBall/tree/main/lib/Ape.js',
	...
	and so can each Ape.Circ be 1 of these, since they form trees.
	...
	...
	*
	...
	FIXME...
	a = Ape.parseApeCode(`(matmulABCLoop *
				matDimA
				matDimC
				(sumInDimBSteps +
					<sumB 0.>
	...
	*/
	Ap: function(name, apeType, childs, data){
		this.name = name;
		...
		this.updateApeSize();
		if(name && name.length == Ape.sizeOfHashId){ //could be a hash name, like created in Ape.prototype.Name(). Verify that name either is that or null (FIXME can it be undefined? probably shouldnt be. What about ''?)
			if(this.Name() != name){
				throw 'Cant have name the same size as hash id (Ape.sizeOfHashId) unless name is its hash id, name='+name+' hash id (generated from content) = '+this.Name();
			}
		}
	},
...
*/
Ape.Ap.prototype.call = function(parrMem, optionalBigMem){
...
//Ape.ApType can be '15' or 'parrMem' or 'f+=' etc. If its '15' this is true. It can be used in loops from 0 to 15-1, in that example, or combined with {* {15} {+ {2} {3}}} etc.
//This does not include '2%5' literals etc. In that example _i/iter is 2 and _s/size is 5, as iter ranges 0 to size-1.
Ape.Ap.prototype.apeTypeIsSimpleSize = function(){
	return Number.isInteger(Number(this.apeType))
};

Ape.Ap.prototype.updateApeSize = function(){
...
//FIXME any code using {} should use this instead so Ape.builtInApeTypesOtherThanPositiveInts.toString etc wont be there for example.
Ape.newEmptyMap = function(){
	let ret = {};
	Object.setPrototypeOf(ret, null);
	return ret;
};

Ape.builtInApeTypesOtherThanPositiveInts = Ape.newEmptyMap();
//floatVal is not actually written. It appears as 2.34 instead. But Ape.parseApeCode('2.34').apeType=='floatVal'.
...
//Once the Ape.Ap language is finised, there might be 30 or so standard apeTypes and no more will be added, plus all the the positive integers as strings.
'ptr f= float floatVal + * 2pow pow triNoDiag triWithDiag union unionMax copy factorial fibonacci threadTimeMemMemMem f* f+= f+ doLast x y z parrMem localparMem bigMem freturn flast iftri'.split(' ').forEach(x=>{
	Ape.builtInApeTypesOtherThanPositiveInts[x] = true;
});

//One of a small set of constant strings or a string of a positive integer less than 2**53 (cuz thats what fits in float64).
Ape.isValidApeType = function(string){
	if(Ape.builtInApeTypesOtherThanPositiveInts[string]){
		return true;
	}
...
//2**46 < 2**53 so it fits in double and the Ape.Ap of the threadTimeMemMemMem defining that calculation would have indexs 0 to (2**46)-1.
//
...
//
Ape.Ap.prototype.toString = function(){
...
Ape.Ap.prototype.size = function(){
	return this.apeSize;
};
...
Ape.Ap.prototype.toApeCode = function(){
	return this.toApeCodeRecurse({});
};

//ns is js {} of string name to Ape.Ap whose .name is that.
...
Ape.Ap.prototype.toApeCodeRecurse = function(ns){
	...
	if(this.apeType == 'floatVal'){
		if(this.data.floatVal === undefined){
			throw 'apeType=floatVal apeType must have Ape.data.floatVal';
		}
...
Ape.oppositeSyntaxChar = {'{':'}', '[':']', '<':'>', '(':')', ')':'(', '>':'<', ']':'[', '}':'{'};

Ape.ParseApe = function(listType){
	this.listType = listType; //One of: [ < { ( l. l means literal.
	this.literal = null;
	this.childs = [];
};
Ape.ParseApe.prototype.toString = function(){
	if(this.listType == 'l'){
		return this.literal;
	}else{
		return this.listType+this.childs.join(' ')+Ape.oppositeSyntaxChar[this.listType];
	}
};

//returns a Ape.Ap
Ape.ParseApe.prototype.toApe = function(){
	return this.toApeRecurse('child',{});
};

//returns a Ape.Ap. The main difference is it has .name and .apeType fields instead of those being the first childs.
//Ape.Ap = function(name, apeType, childs, data)
//syty is 1 of: 'name', 'apeType', or 'child'.
//ns is a namespace, a js {} of string name to Ape.Ap whose .name is that.
//If .name is null then dont put it in the {} and leave it as lazyEval of name to make by hash later if ever.
Ape.ParseApe.prototype.toApeRecurse = function(syty,ns){
	if(Ape.logApeDebug) console.log('toApeRecurse '+this);
	if(syty === undefined){
	...
	if(this.listType == 'l'){
		if(syty == 'name'){
			throw 'Should not convert syty=name literal to Ape. Should convert it to string with literalOrThrow(). this='+this;
			/*if(this.literal == null || this.literal === undefined){
				throw 'syty is name but no ParseApe.literal: '+this;
			}
			...
			if(foundApe){
				...
				//its just name as ParseApe.literal is "sizeA" and finds ns.sizeA is an Ape to reuse.
			...
			*/
			...
			/*
			//throw 'Should not convert an apeType to Ape. apeType is 1 of a small constant set of strings such as f= ptr + * (UPDATE: positive integers can also go there like (matDimA 7))';
			...
			return new Ape.Ap(null, floatArraySizeAsInt, [], {}); //FIXME should it be floatArraySizeAsInt or ''+floatArraySizeAsInt (so apeType is always string)?
			*/
		}else if(syty == 'child'){
			if(this.literal == null || this.literal === undefined){
				throw 'ParseApe.listType is a literal type, but theres no literal. thisParseApe='+this;
			}
			...
			if(/^\d+%\d*$/.test(this.literal)){ //allow second number to be empty, and if so make it 0
				...
				if(Number.isFinite(literalAsNum)){
					return new Ape.Ap(null, 'floatVal', [], {floatVal:literalAsNum});
				}else{
					let name = this.literal;
		...
		let ape = new Ape.Ap(name, this.childs[1].literalOrThrow(), this.childs.slice(2).map(c=>c.toApeRecurse('child',ns)), {});
		ns[name] = ape;
		if(Ape.logApeDebug) console.log('Defined name '+name+' as '+ape.toApeCode());
		return ape;
	}else if(this.listType == '{'){
		return new Ape.Ap(null, this.childs[0].literalOrThrow(), this.childs.slice(1).map(c=>c.toApeRecurse('child',ns)), {});
	}else if(this.listType == '<'){
		return new Ape.Ap(null, 'f=', this.childs.map(c=>c.toApeRecurse('child',ns)), {});
	}else if(this.listType == '['){
		return new Ape.Ap(null, 'ptr', this.childs.map(c=>c.toApeRecurse('child',ns)), {});
	}else{
		throw 'Unknown listType: '+this.listType;
	}
};

Ape.ParseApe.prototype.literalOrThrow = function(){
	if(this.listType != 'l'){
...
/* TODO test this. should be the inverse of toApe.
...
x = Ape.parseApeCode('(matDimA 5)')

Ape.ParseApe {listType: '(', literal: null, childs: Array(2)}
y = x.toApe('child')
Ape.Ap {name: 'matDimA', apeType: '5', childs: Array(0), data: {…}, apeSize: 5}
y.toApeCode()
'(matDimA 5)'
6[Violation] 'requestAnimationFrame' handler took <N>ms
x = Ape.parseApeCode('(varX float)').toApe('child')

Ape.Ap {name: 'varX', apeType: 'float', childs: Array(0), data: {…}, apeSize: 1}
x = Ape.parseApeCode('(varX float)').toApe('child').toApeCode()

'(varX float)'
x = Ape.parseApeCode('[a (varX float) c]').toApe('child').toApeCode()

THIS PART IS WRONG[
'[(null float) (varX float) (null float)]'
x = Ape.parseApeCode('[(a 2) (varX float) (c 3)]').toApe('child').toApeCode()
]

'[(a 2) (varX float) (c 3)]'
x = Ape.parseApeCode('[(a 2) (varX float) (c 3)]').toApe('child').childs[0].toApeCode()

'(a 2)'
x = Ape.parseApeCode('[(a 2) (varX float) (c 3)]').toApe('child').childs[1].toApeCode()

'(varX float)'
x = Ape.parseApeCode('[(a 2) (varX float) (c 3)]').toApe('child').childs[2].toApeCode()

'(c 3)'
*/
Ape.Ap.prototype.toParseApe = function(){
	let parseApe = new Ape.ParseApe();
	if(this.apeType == 'floatVal'){ //unlike (varName float) which is a float var that goes in a memory branch but not a specific float value (which goes in time/code area).
		let literalParseApeOfFloatAsString = new Ape.ParseApe();
		...
		if (this.name){
			parseApe.listType = '(';
			let nameParseApe = new Ape.ParseApe();
		...
		let typeParseApe = new Ape.ParseApe();
...
//returns a Ape.ParseApe, which you should then convert to a Ape.Ap, and then in some cases to tinyGlsl code.
Ape.parseApeCode = function(apeCode){
	return Ape.parseApeTokens(Ape.tokenizeApeCode(apeCode));
};

//js [] list of tokens. Example tokens: [ ] < >  { } ( ) ptr f= varName55
Ape.parseApeTokens = function(apeTokens){
	return Ape.parseApeTokensRecurse(apeTokens, new Set());
};

/*Ape.parseApeTokensRecurse = function(apeTokens, set){
	throw 'TODO';
};*/
/*Ape.parseApeTokensRecurse = function(apeTokens, set) {
	...
	switch(token) {
		case '[':
			parseApe.listType = '[';
			while(apeTokens[0] !== ']') {
				parseApe.childs.push(Ape.parseApeTokensRecurse(apeTokens, set));
			}
		...
		case '{':
			parseApe.listType = '{';
			parseApe.ApType = apeTokens.shift(); // Assume next token is the apeType
			while(apeTokens[0] !== '}') {
				parseApe.childs.push(Ape.parseApeTokensRecurse(apeTokens, set));
			}
			apeTokens.shift(); // Pop the '}'
		...
		case '(':
			...
			parseApe.ApType = apeTokens.shift(); // Next token is the apeType
			while(apeTokens[0] !== ')') {
				parseApe.childs.push(Ape.parseApeTokensRecurse(apeTokens, set));
			}
			apeTokens.shift(); // Pop the ')'
		...
		case '{':
			...
			while (apeTokens[0] !== '}') {
				if (apeTokens[0] === '{') {
					...
					let innerApe = new Ape.ParseApe();
					innerApe.listType = 'l';
					innerApe.literal = apeTokens.shift();
...
Ape.parseApeTokensRecurse = function(apeTokens, set) {
	...
	let token = apeTokens.shift(); // Pop the first token
	let parseApe = new Ape.ParseApe(); //FIXME give listType in constructor of parseape?
...
Ape.tokenizeApeCode = function(apeCode){
...
Ape.Ap.prototype.sharedArrayName = function(){
	if(this.apeType != 'ptr'){
...
Ape.Ap.prototype.outermostIsDeclaringMem = function(){
...
Ape.Ap.prototype.ptrCode = function(){
...
Ape.Ap.prototype.contentToHash = function(){
...
Ape.Ap.prototype.Name = function(){
	if(!this.name){
		if(this.apeType == 'floatVal'){
			this.name = Ape.syncTypeToChar.const+this.data.floatVal; //FIXME should it just be '2.34' if thats the floatVal?
		}else{
			this.name = Ape.syncTypeToChar.const+Ape.sha256HexOfString(this.contentToHash());
		}
	}
	return this.name;
};
...
Ape.Ap.prototype.OffsetName = function(){
	return this.Name()+'_o';
};

Ape.Ap.prototype.SizeName = function(){
...
Ape.Ap.prototype.LoopName = function(){
	/*if(this.apeType == 'floatVal'){ //FIXME?
		if(Math.floor(this.data.floatVal) != this.data.floatVal) throw 'Not an integer '+this.data.floatVal;
		return this.data.floatVal;
	}*/
...
Ape.Ap.prototype.ArrayName = function(){
	return this.Name()+'_a';
};

Ape.Ap.prototype.FloatName = function(){
	return this.Name()+'_f';
};
...
Ape.Ap.prototype.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc = function(tablev, opString){
	let code = '';
	...
	for(let i=0; i<this.childs.length; i++){
		...
		//if(multiLine) code += Ape.nline(tablev+1)+ch.toTinyGlslCodeRecurse(tablev+1);
		if(multiLine) code += (i>0 ? Ape.nline(tablev+1) : '')+ch.toTinyGlslCodeRecurse(tablev+1);
		else code += ch.toTinyGlslCodeRecurse(0).trim();
	}
	if(multiLine) code += Ape.nline(tablev);
	code = code.trim();
	if(this.childs.length > 1) code += ')';
	return code;
	//return code.trim();
...
Ape.Ap.prototype.toTinyGlslCodeRecurse_tanhPowSinExpEtc = function(tablev, glslFuncName){
	//return glslFuncName+'('+this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev, ',')+')';
	return glslFuncName+this.toTinyGlslCodeRecurse_listOfMultiplyPlusEtc(tablev, ',');
};

Ape.Ap.prototype.updateEdGroups = function(circ){
	//console.log('Updating Eds and EdGroups in circ='+circ);
	if(Ape.logEdGroupStuff) console.log('Updating Eds and EdGroups in a circ');
	let ret = []; //list of edGroups, existing or new.
	let parrMems = this.searchApes(ape=>(Ape.ApType=='parrMem'));
	...
	while(circ.edsOut.length < targetNumEds){
		circ.edsOut.push(new Ape.Ed(circ, 0, 0, 0, 0, 0)); //FIXME update those numbers
	}
	let edIndex = 0;
	for(let ape of parrMems){
		...
		if(existingEdGroup && existingEdGroup.eds.length == targetNumEdsInThisGroup){
			...
			if(Ape.logEdGroupStuff) console.log('Not changing existingEdGroup='+existingEdGroup);
			...
			let newEdGroup = new Ape.EdGroup('edGroup for '+ape, eds); //sets each ed.edGroup, so that hooks it into the circ at circ.edsOut[index].edGroup.
			ret.push(newEdGroup);
			...
			if(Ape.logEdGroupStuff) console.log('Making edGroup for ape='+ape+' in circ.eds['+edIndex+'..'+(edIndex+targetNumEdsInThisGroup-1)+'], newEdGroup='+newEdGroup);
		}
...
Ape.Ap.prototype.toTinyGlslCode = function(){
	let code = '';
	let parrMems = this.searchApes(ape=>(Ape.ApType=='parrMem'));
	let localparMems = this.searchApes(ape=>(Ape.ApType=='localparMem'));
	let floatMems = this.searchApes(ape=>(Ape.ApType=='float'));
	let tablev = 0;
	...
	for(let parrMemApe of parrMems){
		...
		//code += Ape.nline(tablev)+'parrMem TODO: '+parrMemApe.Name()+'['+parrMemApe.size()+']';
		let size = parrMemApe.size();
		code += Ape.nline(tablev)+'const int '+parrMemApe.OffsetName()+' = '+parrMemOffset+'; //array offset in parrMem';
		code += Ape.nline(tablev)+'const int '+parrMemApe.SizeName()+' = '+size+'; //array size in parrMem';
		parrMemOffset += size;
	...
	for(let localparMemApe of localparMems){
		//TODO merge duplicate code between parrMem, localparMem, and maybe bigMem
		//code += Ape.nline(tablev)+'localparMem TODO: '+localparMemApe.Name()+'['+localparMemApe.size()+']';
		let size = localparMemApe.size();
		code += Ape.nline(tablev)+'const int '+localparMemApe.OffsetName()+' = '+localparMemOffset+'; //array offset in localparMem';
		code += Ape.nline(tablev)+'const int '+localparMemApe.SizeName()+' = '+size+'; //array size in localparMem';
	...
	for(let floatMemApe of floatMems){
		//code += Ape.nline(tablev)+'floatMem TODO: '+floatMemApe.Name();
		code += Ape.nline(tablev)+'float '+floatMemApe.FloatName()+' = 0.;';
	}
...
Ape.Ap.prototype.toTinyGlslCodeRecurse = function(tablev){
	let code = '';
	if(this.apeType == '*'){
		for(let c=0; c<this.childs.length-1; c++){
			let ch = this.childs[c];
			//code += Ape.nline(tablev+c)+'for(int '+ch.LoopName()+'=0; '+ch.LoopName()+'<'+ch.SizeName()+'; '+ch.LoopName()+'++){';
			code += Ape.nline(tablev+c)+'for(int '+ch.LoopName()+'=0; '+ch.LoopName()+'<'+ch.size()+'; '+ch.LoopName()+'++){';
		}
		...
		for(let c=this.childs.length-2; c>=0; c--){
			let ch = this.childs[c];
			code += Ape.nline(tablev+c)+'}';
			if(Ape.toTinyGlslCode_putsLoopNameInClosingBraceComment){
				code += '//'+ch.Name();
			}
		}
		//code += Ape.nline(tablev)+'for(let '+this.Name()+'=0; '+this.Name()+'<TODO; '+this.Name()+'++){';
		...
		//code += Ape.nline(tablev+1)+'TODO inner loops/forks'+n;
		//code += Ape.nline(tablev)+'}'+n;
	}else if(this.apeType == 'iftri'){
		code += Ape.nline(tablev)+'if('+this.childs[0].LoopName()+'<'+this.childs[1].LoopName()+'){ //iftri. TODO optimize this as triangle loop if its parents can';
		code += this.childs[2].toTinyGlslCodeRecurse(tablev+1);
		code += Ape.nline(tablev)+'}';
	}else if(this.apeType == '+'){
		for(let child of this.childs){
			if(child.outermostIsDeclaringMem()){
				code +=  Ape.nline(tablev)+'//outermostIsDeclaringMem '+child.Name();
			}else{
		...
		//code += Ape.nline(tablev)+this.childs[0].ptrCode()+' = '+this.childs[1].toTinyGlslCodeRecurse(tablev+1).trim()+';'; //FIXME make a func that does it in 1 line
		code += Ape.nline(tablev)+this.childs[0].toTinyGlslCodeRecurse(tablev+1)+' = '+this.childs[1].toTinyGlslCodeRecurse(tablev+1).trim()+';'; //FIXME one line or multiline depending on length
	}else if(this.apeType == 'f+='){
		code += Ape.nline(tablev)+this.childs[0].ptrCode()+' += '+this.childs[1].toTinyGlslCodeRecurse(tablev+1).trim()+';'; //FIXME make a func that does it in 1 line
	}else if(this.apeType == 'f*='){
		...
		code += Ape.nline(tablev)+this.childs[0].ptrCode()+' /= '+this.childs[1].toTinyGlslCodeRecurse(tablev+1).trim()+';';
	}else if(this.apeType == 'f+'){
		...
		*/
		...
		code += Ape.nline(tablev)+this.childs[0].toTinyGlslCodeRecurse(tablev)+'/'+this.childs[1].toTinyGlslCodeRecurse(tablev+1).trim();
		...
		//code += Ape.nline(tablev)+this.FloatName();
		code += this.FloatName(); //FIXME nline?
	}else if(this.apeType == 'floatVal'){
		code += Ape.jsNumToGlslFloat(this.data.floatVal);
	}else if(this.apeType == 'freturn'){
		if(this.childs.length != 1){
			throw 'x. Must be 1 childs if apeType freturn but is '+this.childs.length;
		}
		code += Ape.nline(tablev)+'return '+this.childs[0].toTinyGlslCodeRecurse(0).trim()+';'; //FIXME make a func that does it in 1 line
		...
		//TODO generalize this to the oGlo/nGlo vars so its not specific to x y and z. More generally, Ape.Ap is many GPU codes run on many shared vars in many combos,
	...
	if(!code){
		code = Ape.nline(tablev)+'TODO apeType='+this.apeType+' toTinyGlslCodeRecurse here, ape='+Ape.limitStringLen(this.toApeCode(),35); //TODO make sure this toApeCode is 1 line
	}
	return code;
};

//returns a list of Ape matching query(ape)->true in depthFirst order
//Example: ape.seachApes(ape=>(Ape.ApType=='parrMem'))
Ape.Ap.prototype.searchApes = function(query){
	let list = [];
...
Ape.Ap.prototype.searchApesRecurse = function(query, list, set){
	for(let child of this.childs){
...
Ape.Ap.prototype.indexCode = function(){ //FIXME i think this has been replaced by ape.LoopName()??
	return this.name+'_i'; //FIXME check if this duplicates another name
};

Ape.Ap.prototype.code = function(){
	throw 'TODO';
};

Ape.doSimpleApeTests = function(){
	console.log('START Ape.doSimpleApeTests()');
	...
	for(let apeCode of apeCodes){
		...
		let parseApe = Ape.parseApeCode(apeCode);
	...
	console.log('END Ape.doSimpleApeTests(), all tests pass.');
};
...
Dagball056.html:3666 END Ape.doSimpleApeTests(), all tests pass.
...
Ape.parseApeCode('{* {5} (sizeB 2)}').toApe().contentToHash()
'{"type":"ape","apeType":"*","data":{},"childs":["c1efccd6afd44eede3f5fdec9fe2956d3f5ad80a8c3d277e2a28f2649aca7c44a","sizeB"]}'
Ape.parseApeCode('2.34').toApe().contentToHash()
'{"type":"ape","apeType":"floatVal","data":{"floatVal":2.34},"childs":[]}'
Ape.parseApeCode('2.34').toApe().Name()
'c2.34'
Ape.parseApeCode('{* {5} (sizeB 2)}').toApe().Name()
'c93f881ed79ef1e9d1a9201eb52c0a96416e4f40a0e9671ddeb8b6d5b6082f31f'
Ape.parseApeCode('(mulXX * {5} (sizeB 2))').toApe().Name()
'mulXX'
Ape.parseApeCode('(mulXX * {5} (sizeB 2))').toApe().childs[0].Name()
'c1efccd6afd44eede3f5fdec9fe2956d3f5ad80a8c3d277e2a28f2649aca7c44a'
Ape.parseApeCode('(mulXX * {5} (sizeB 2))').toApe().childs[0].toApeCode()
'{5}'
*/

if(Ape.whenBoot_doSimpleApeTests){
	Ape.doSimpleApeTests();
}

//Ape.Ap.prototype.codeToReadFloat = function()
...
//Each Ape.Circ should give 1 parrMem, 1 localparMem, and 1 stackMem.
//Some parts will be generated but do need to be in the math model. All the varNames for a Ape.Circ
//will be prefixed by something unique to that circ so they dont overlap vars from other circs.
*/


console.log('Ape.doSimpleApeTests()...');
Ape.doSimpleApeTests();

/*
Ape found TinyGlsl = [object Object] (else throw cuz not found)
Ape.js:1522 Ape.doSimpleApeTests()...
Ape.js:1256 START Ape.doSimpleApeTests()
Ape.js:1277 
Testing apeCode=(sizeB 5)
Ape.js:1279 parseApe=(sizeB 5)
Ape.js:1282 backToApeCode=(sizeB 5)
Ape.js:1287 Test pass, apeCode=(sizeB 5)

Ape.js:1277 
Testing apeCode=(matAB * (sizeA 3) sizeA)
Ape.js:1279 parseApe=(matAB * (sizeA 3) sizeA)
Ape.js:1282 backToApeCode=(matAB * (sizeA 3) sizeA)
Ape.js:1287 Test pass, apeCode=(matAB * (sizeA 3) sizeA)

Ape.js:1277 
Testing apeCode=[(sizeA 2) (sizeB 3) (sizeC 4)]
Ape.js:1279 parseApe=[(sizeA 2) (sizeB 3) (sizeC 4)]
Ape.js:1282 backToApeCode=[(sizeA 2) (sizeB 3) (sizeC 4)]
Ape.js:1287 Test pass, apeCode=[(sizeA 2) (sizeB 3) (sizeC 4)]

Ape.js:1277 
Testing apeCode=(matAB * (sizeA 5) (sizeB 7))
Ape.js:1279 parseApe=(matAB * (sizeA 5) (sizeB 7))
Ape.js:1282 backToApeCode=(matAB * (sizeA 5) (sizeB 7))
Ape.js:1287 Test pass, apeCode=(matAB * (sizeA 5) (sizeB 7))

Ape.js:1277 
Testing apeCode={* (sizeA 5) (sizeB 7)}
Ape.js:1279 parseApe={* (sizeA 5) (sizeB 7)}
Ape.js:1282 backToApeCode={* (sizeA 5) (sizeB 7)}
Ape.js:1287 Test pass, apeCode={* (sizeA 5) (sizeB 7)}

Ape.js:1277 
Testing apeCode={5}
Ape.js:1279 parseApe={5}
Ape.js:1282 backToApeCode={5}
Ape.js:1287 Test pass, apeCode={5}

Ape.js:1277 
Testing apeCode={* {5} (sizeB 7)}
Ape.js:1279 parseApe={* {5} (sizeB 7)}
Ape.js:1282 backToApeCode={* {5} (sizeB 7)}
Ape.js:1287 Test pass, apeCode={* {5} (sizeB 7)}

Ape.js:1277 
Testing apeCode={* {5} {7}}
Ape.js:1279 parseApe={* {5} {7}}
Ape.js:1282 backToApeCode={* {5} {7}}
Ape.js:1287 Test pass, apeCode={* {5} {7}}

Ape.js:1277 
Testing apeCode={* (sizeA 5) {7}}
Ape.js:1279 parseApe={* (sizeA 5) {7}}
Ape.js:1282 backToApeCode={* (sizeA 5) {7}}
Ape.js:1287 Test pass, apeCode={* (sizeA 5) {7}}

Ape.js:1277 
Testing apeCode=2.34
Ape.js:1279 parseApe=2.34
Ape.js:1282 backToApeCode=2.34
Ape.js:1287 Test pass, apeCode=2.34

Ape.js:1277 
Testing apeCode=<(varB float) 2.34>
Ape.js:1279 parseApe=<(varB float) 2.34>
Ape.js:1282 backToApeCode=<(varB float) 2.34>
Ape.js:1287 Test pass, apeCode=<(varB float) 2.34>

Ape.js:1277 
Testing apeCode=<(varB float) (varC float)>
Ape.js:1279 parseApe=<(varB float) (varC float)>
Ape.js:1282 backToApeCode=<(varB float) (varC float)>
Ape.js:1287 Test pass, apeCode=<(varB float) (varC float)>

Ape.js:1277 
Testing apeCode=[(matAB * (sizeA 5) (sizeB 7)) sizeA sizeB]
Ape.js:1279 parseApe=[(matAB * (sizeA 5) (sizeB 7)) sizeA sizeB]
Ape.js:1282 backToApeCode=[(matAB * (sizeA 5) (sizeB 7)) sizeA sizeB]
Ape.js:1287 Test pass, apeCode=[(matAB * (sizeA 5) (sizeB 7)) sizeA sizeB]

Ape.js:1277 
Testing apeCode={x}
Ape.js:1279 parseApe={x}
Ape.js:1282 backToApeCode={x}
Ape.js:1287 Test pass, apeCode={x}

Ape.js:1277 
Testing apeCode={y}
Ape.js:1279 parseApe={y}
Ape.js:1282 backToApeCode={y}
Ape.js:1287 Test pass, apeCode={y}

Ape.js:1277 
Testing apeCode={z}
Ape.js:1279 parseApe={z}
Ape.js:1282 backToApeCode={z}
Ape.js:1287 Test pass, apeCode={z}

Ape.js:1277 
Testing apeCode={f+ {x} {y} {z} 1000}
Ape.js:1279 parseApe={f+ {x} {y} {z} 1000}
Ape.js:1282 backToApeCode={f+ {x} {y} {z} 1000}
Ape.js:1287 Test pass, apeCode={f+ {x} {y} {z} 1000}

Ape.js:1289 END Ape.doSimpleApeTests(), all tests pass.
*/