...
Ap.Ape.prototype.transformNames = function(renamer){
...
Ap.Ape.prototype.prefixNames = function(prefix){
	return this.transformNames(name=>(prefix+name));
};
...
Ap.Ape.prototype.eval = function(numGpuThreads, floatsPerGpuThread, parMem, bigMem){
	let tinyglslCode = this.toTinyGlslCode();
	...
	let floatsOut = TinyGlsl.simple('//TODO prefix code', adjustedTinyglslCode, parMem, bigMem, height, width, floatsPerGpuThread);
	return floatsOut;
};
...
Ap.Ape.prototype.toTinyGlslCode = function(){
	let code = '';
	let parrMems = this.searchApes(ape=>(Ap.ApeType=='parrMem')); //FIXME theres parMem locMem parlocMem locparMem bigMem.
