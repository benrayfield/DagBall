	...
	*/
	...
	simple:
		...
		(function(beforeCode, code, par, height, width){
			...
			if(!outs){
				outs = TinyGlsl.caches[arrCacheKey] = TinyGlsl.putTid(new Float32Array(height*width)); //would automaticaly putTid if use TinyGlsl.cache(key, lazyEvalIfKeyNotFound)
			}
