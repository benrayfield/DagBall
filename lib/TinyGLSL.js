	...
	*/
	...
	simple:
		...
		(function(beforeCode, code, par, big, height, width, optionalFloatsOutPerPixel){
			...
			return TinyGlsl.internalGLSL_disorganizedTODO(code3, par, big, outs, height, width);
		}),
	...
	*/
	...
	internalGLSL_disorganizedTODO:
		(function(glslCode, floatsPar, bigMem, floatsOutOrOutputSize, canvasHeight, canvasWidth){
			...
			if(bigMem){
				throw 'TODO upgrade TinyGlsl to have bigMem. As of 2023-11-3 its for future expansion and will use isample2D or texture or buffer or something.';
			}
			...
			*/
			...
			let gl = TinyGlsl.cache('gl'+cacheKeySuffix, function(){
				...
				console.log('Number of gl contexts before create another: '+Object.keys(TinyGlsl.caches).filter(x=>x.startsWith('gl_')).length);
				let gl = canvas.getContext(TinyGlsl.webglType);
				glErr = gl.getError();
				if(glErr){
					throw 'after canvas.getContext(TinyGlsl.webglType), gl.getError()='+glErr;
				}
